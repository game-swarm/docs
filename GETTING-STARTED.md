...
基础启动顺序为：先启动 NATS，再启动 `sandbox` worker、`engine`、`gateway`，最后启动 `frontend`。Engine、Gateway 和 Sandbox worker 都会重试初始 NATS 连接；NATS 是 canonical sandbox transport，不存在无 NATS 的本地 sandbox fallback。每个仓库的 README 负责说明本仓库的本地启动命令。
