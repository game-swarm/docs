...
5. **开源首日**：MIT 许可证。
6. **设计即终态**：设计文档只描述已裁定的目标状态。禁止以"远期方向"、Phase、future、deferred、以后再说、版本分期等方式推迟设计决策；每个设计决策必须按当下最佳实践一次性裁定。禁止基于工程难度、工期或工作量做分期取舍——当下能裁定的必须在当下裁定。仅硬外部条件（playtest 数据、外部 API 可用性等实证依赖）可gate 数值校准和平衡性验证。实现顺序只属于 ROADMAP，不得写进 DESIGN/spec/reference。
