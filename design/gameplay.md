...
> 以下数值由 Resource Ledger §Empire Upkeep 公式 `upkeep = base_upkeep × rooms × (1 + rooms / room_soft_cap)` 派生，Standard 默认 `base_upkeep=30, room_soft_cap=15`。**权威数值见 [Economy Balance Sheet](economy-balance-sheet.md) §1 Maintenance Curve**。

```text
小帝国（1 房）: 维护费 ≈ 32/tick     — 基础代码正流
中帝国（5 房）: 维护费 ≈ 200/tick    — 普通代码可承受
大帝国（20 房）: 维护费 ≈ 1,400/tick — 需要高效经济
巨帝国（50 房）: 维护费 ≈ 6,500/tick — 顶级代码区间
```
