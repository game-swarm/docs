...
**目标态**允许世界配置扩展身体部件、建筑、伤害、action 与资源。当前 `WorldConfig` 使用 `[[body_part_types]]` / `[[structure_types]]` / `[[damage_types]]` / `[[resource_types]]` 和 `[[custom_actions]]`；vanilla action 由引擎预注册且不能通过 TOML 覆盖。
...
| `transfer_to_global_fee_bp` | BasisPoints | `100` | 本地→全局转换费率（默认 1%） |
| `global_deposit_delay` | u32 | 10 | 转换所需的 tick 数（不可为 0，防止瞬移补给） |
| `transfer_from_global_fee_bp` | BasisPoints | `500` | 全局→本地转换费率（默认 5%） |
...
| 市场交易 | Extension surface | — | 不属于 active Resource Ledger |
| Controller passive income | Faucet | 由 Resource Ledger 配置 | 新手经济基础收入，保证 1-3 房基础代码正流 |
| Wreckage salvage | Faucet | 由 Resource Ledger 预算限制 | 被摧毁 drone 掉落残骸；回收效率低于 Recycle，随 tick 衰减 |
...
| **伤害类型** | 6 种：`Kinetic`, `Thermal`, `EMP`, `Sonic`, `Corrosive`, `Psionic` | 默认抗性均为 1.0，详见本节伤害与武器类型定义 |
| **物流模式** | **模式 B（轻物流）** | 全局传输 1% 损耗，本地建造 5% 损耗。模式 A（无损耗）和模式 C（重物流）为可选项 |
| **Vanilla Action** | 包含 11 种官方 action：3 种基础 combat（`Attack`, `RangedAttack`, `Heal`）+ 8 种 special action（`Hack`, `Drain`, `Overload`, `Debilitate`, `Disrupt`, `Fortify`, `Leech`, `Fabricate`）。全部通过 `ActionRegistry` dispatch；世界模式通过 `mods.lock` typed allowlist 控制 Tutorial/Novice/Standard 可用集合 | 冷却时间与资源消耗见 canonical Action 表格 |
...
| **展示/世界统计** | World 模式无公开排行榜；`swarm_get_world_statistics` 只发布非竞争、隐私过滤后的世界统计；Arena 段位统计属于竞技场输出面 | 持久世界天然不公平（老玩家先发优势），竞技场模式为有限时间窗口的公平竞争 |
...
soft_launch 结束 (tick T):
  ...
  │   ├─ 伤害倍率：`damage_multiplier = 0.5`（world.toml decimal；引擎内部换算为 5000 bp）
...
| Heal | —（反向治疗） | 12 | 治疗（距离 1），每 tick 可缩短一个负面状态 10 tick 持续时间 |
...
| **Leech** | Attack | 吸血攻击：参数见 canonical 表 | 见 canonical 表 | 见 canonical 表 | 见 canonical 表 |
| **Fabricate** | Work+Carry | drone→建筑转化：将敌方 drone 转化为己方结构（Tower/Storage/Wall），需持续 channel；Vanilla 成本为纯 Energy | 见 canonical 表 | 5000 Energy | 见 canonical 表 |
...
特殊攻击的世界层级选择由 mod runtime 的 typed allowlist 控制；`world.toml` 不承载 `vanilla.special_attacks_enabled` 控制面。
...
- 引入**新 action** 时（如服主自定义 `Scramble`），需在 `ActionRegistry` 注册 action type + 对应 validate/apply handler；内部 IDL 使用通用 `Action { action_type, payload }`，wire `type` 为具体 action 名称。`Leech` 是 vanilla ActionRegistry action，不是新的 CommandAction。
...
- 一个 action type 可被多个 body part 触发（如 `Move` 只能由 `Move` part 触发，但 `Attack` 在后续可由 `Claw`/`Bite` 等多个 part 触发）
- 新 body part 绑定到**已有 action type** 时，只需定义不同的 damage_type/base_damage/cost ——引擎自动复用该 ActionRegistry handler 的校验和应用逻辑
- 引入**新 action** 时（如服主自定义 `Scramble`），需在 `ActionRegistry` 注册 action type + 对应 validate/apply handler；内部 IDL 使用通用 `Action { action_type, payload }`，wire `type` 为具体 action 名称。`Leech` 是 vanilla ActionRegistry action，不是新的 CommandAction。
...
3. 自动暴露通用 API 面——runtime ActionRegistry 中的 custom action 会进入 runtime `IdlDoc` 和生成的 SDK；MCP 使用允许非保留 action 名称的通用 custom-action schema，不为每个 action 新增工具
4. WASM 模块通过 tick() → CommandIntent[] 使用新 action（与内置 action 语法一致）
```
...
| 字段 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `name` | string | ✅ | 唯一 action type；进入内部 `Action.action_type`，并作为 wire `type` |
| `description` | string | ✅ | 人类可读描述 |
...
与 body_part_types 和 damage_types 一样，特殊效果可通过 world.toml 定义和扩展。每个 `[[special_effects]]` 条目定义一个可由 `[[custom_actions]].special_effect` 引用的效果类型：
...
# 以下 8 个 special action 的 Tutorial/Novice/Standard 可用集合由 mods.lock typed allowlist 控制。
...
```typescript
// TypeScript SDK — tick() 接收 Snapshot，返回 CommandIntent[]
function tick(snapshot: WorldSnapshot): CommandIntent[] {
...
Plugin 可向 Bevy schedule 注入 systems，不限制为固定 hook surface。所有 Plugin systems 必须进入 Complete Tick Execution Manifest 的 R/W matrix 与 plugin schedule graph。新增 CommandAction、SpecialEffect handler、body part、damage type 或 resource operation 时，Plugin 通过引擎注册表声明 schema 与 handler；注册结果纳入 `world_action_manifest_hash`、`plugin_schedule_graph_hash` 和 replay 输入。模组以源码分发，服务器主人在编译进 Engine 前审查代码；安全边界在源码审查、签名包和编译清单层。
...
> **免维护费**：前 1 controller + 3 drone 在 `free_upkeep_ticks`（默认 2000）内免维护费；PvP 开启由 `pvp_unlock_tick` 独立配置。新玩家初始资源 `{Energy: 5000}`（Vanilla 默认单一 Energy；多资源属于 mod/advanced worlds）。详见 Resource Ledger §2.3。
...
**资源交换边界**：消息 payload 可表达 offer 或协商意图，但不改变 Resource Ledger。任何资源交换必须通过 Transfer Gateway（LocalTransfer、GlobalDeposit/Withdraw、AlliedTransfer 等 active resource operation）执行；消息不能绕过费用、延迟、transfer lock、visibility 或 TickTrace 归因。
...
**WASM 消息接口（IDL vNext 草案）**：新的 IDL 版本可为 tick 输出增加 `messages` 字段。canonical v1 recovery 路径保持 `CommandIntent[]`，以下为 vNext envelope 草案：

```typescript
// Future IDL proposal — not accepted by the current engine
function tick(snapshot: WorldSnapshot): FutureTickResult {
    const commands: CommandIntent[] = [...];
...
引擎在 EXECUTE 阶段处理 commands 的同时处理 messages，并在下一 tick snapshot 中暴露给接收方 WASM。canonical v1 客户端继续使用直接 `CommandIntent[]` 输出；消息读取由 Registry 中发布的 MCP 工具提供恢复与兼容路径。
