...
**Age 恢复**: drone 必须**移动到 Controller 或 Forward Depot 维修范围内**才能降低 age。Controller 维修距离随 RCL 增长（RCL1=1 格，RCL8=5 格），免费，每 tick 服务上限由 RCL 决定。Forward Depot 固定 range=1，消耗存储资源。相邻格只有 6 个——大量 drone 需要排队，形成物流拥挤决策。维修能力受物理范围（repair_range）和 per-Controller 容量（repair_capacity）自然限制——多 Controller 在不同房间维修不同 drone，不存在额外的全局 cap。**Healer body part 只能恢复 HP，不能降低 age。
...
**Deterministic action log model**：模组通过 `state` 获取只读 snapshot → 产出 `action log`（`actions.emit_event` 等的序列化记录）→ 引擎在模组执行完毕后 validate logs（校验 player_id 合法性、resource 存在性、amount 非溢出）→ 所有模组的 action logs 经 deterministic merge（按模组声明顺序合并，同 target 的先后 action 保留顺序）→ apply once（在 death_cleanup 之后一次性写入 ECS）。模组不得直接修改 World 状态——所有变更必须通过 actions 接口进入 action log 管线。
...
**State iterator order fixed**：`state.players()`、`player.drones()`、`player.rooms()` 的迭代顺序固定——`players()` 按 `player_id` 字典序；`drones()` 按 `(room_id, entity_id)` 元组序；`rooms()` 按 `room_id` 字典序。使用 `BTreeMap` 确保跨平台一致。
...
**模组扩展**: 伤害类型、抗性和属性倍率通过 world.toml 声明式配置扩展；Rhai 模组运行期只能使用 `specs/reference/rhai-mod-abi.md` 定义的 5 个 `actions.*` API 产生日志化副作用。
...
**模组扩展**：Rhai 模组可通过声明式配置把新 body part 绑定到已有 ActionRegistry action：

```rust
// world.toml 声明式扩展：新 body part 复用 vanilla ActionRegistry action
actions.add_body_part_type("Leech", #{
    action: "Leech",           // vanilla ActionRegistry action
...
| **能力白名单** | 所有 `actions.*` 调用经引擎白名单校验 | 未注册的 action 被拒绝 + 审计日志。当前白名单以 `specs/reference/rhai-mod-abi.md` 为准：deduct_resource/award_resource/damage_entity/emit_event/log_info |
...
| `log_info` | `engine::rule::handler::log_info` | 日志输出 |
