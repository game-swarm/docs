...
> 游戏机制域文件。从 design/README.md 拆分。Vanilla Ruleset、身体部件、伤害系统、特殊攻击、经济模型。
...
> **RFC 占位——不在当前设计范围内**。Market、Terminal 交易、全局订单簿、撮合引擎等均不在当前设计范围内。所有市场功能引用标记为 RFC，待独立经济模拟闭环完成后再评估。
...
Market 和 trading 功能为 **RFC 占位——不在当前设计范围内**。已从 IDL/默认 SDK 移除，仅保留 RFC 文档。未来方向为 Transfer + Alliance 资源池的上层抽象（与 R14 D3 裁决一致）。不包含价格发现、撮合引擎、跨世界结算。所有 market 相关文档引用标注 `⏳ RFC`。
...
| 贸易/市场解锁门控 | `new_player_market_lock_ticks` | 500 tick | 预留参数——Market 为 RFC，当前从 IDL 移除。若后续冻结 Market，此参数控制新玩家市场准入 |
| PvE drop 绑定 | `new_player_pve_drop_bound` | true | 前 N tick PvE 掉落绑定到账号，不可交易/转移 |
| 同源账号组配额 | `same_origin_account_group_quota` | 5 | 同一 IP/device fingerprint 的账号组共享此配额，超限账号标记为受限 |
...
# market_requires_terminal: RFC 占位——市场功能不在当前设计范围内
...
- `swarm mod add` 和 `swarm mod update` 自动将当前 checkout 的 commit hash 写入 `mods.lock`
- engine 启动时以 `mods.lock` 为准进行 checkout；若所在 commit 与 `world.toml` 声明的 tag 不对应（tag 被 force-push），**拒绝启动**（不是 warn），返回 `ModCommitMismatchError`
- `mods.lock` 必须含 content hash（`checksum` 字段，sha256）；校验失败 → `ModIntegrityError`
...
**默认启用**：empire-upkeep 模组通过 protocol hook + Vanilla 公式默认启用。服主可通过 `world.toml` 关闭（注释对应 `[[mods]]` 条目）或替换为自定义维护费公式。
