...
Arena **P0 以房间制比赛为核心**——玩家创建比赛房间，设定参数，自己或他人加入，比赛结束后生成 room match_result（赛果摘要）。无需自动匹配、天梯排名、赛季组织等上层编排。Arena 的定位是**轻量房间制测试场**：玩家用 WASM 算法在隔离房间中对战，赛后获得 match_result 反馈。
