...
|-----|----|------|------|---------|------|
| **Creep**（野怪） | 50 | 10/tick | 固定巡逻路径（房间内），攻击进入视野的 drone | 50 tick | 引用 Resource Ledger `PvEAward` T1 low roll |
| **Guardian**（远古守卫） | 300 | 30/tick | 驻守固定位置（资源据点），不巡逻 | 300 tick | 引用 Resource Ledger `PvEAward` T2/T3 |
...
NPC 掉落金额引用 Resource Ledger `PvEAward` tier 表：Creep 映射为 T1 low roll，Guardian 按模板强度映射为 T2/T3。具体金额由 `PvEAward` 预算裁决，受玩家、区域、全局和事件预算共同约束；本设计文档不另行定义独立数值表。
