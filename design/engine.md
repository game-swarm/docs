...
**Replay 分层**：确定性 replay 只需要 redb 中的 replay-critical TickCommitRecord core（见 `specs/core/05-persistence-contract.md` §2.1）和 keyframe/delta chain。`TickInputEnvelope` 记录 COLLECT 输入与运行环境，Replay identity 记录 collect/attempt/commit 与规则版本，RichTraceBlob 仅用于 rich debug replay。Object Store/RichTraceBlob 缺失产生 `audit_gap`，不影响 deterministic replay。
