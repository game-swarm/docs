...
> 引擎架构域文件。从 design/README.md 拆分。详细规范见 `../specs/core/tick-protocol.md`。
...
# 服主部署锁文件（权威运行时配置）
[plugins.empire-upkeep]
enabled = true
version = "2.0.0"

[plugins.empire-upkeep.config]
drone_cost = 2
room_base = 10
```

`world.toml` 描述世界规则；`mods.lock` 描述部署选择、版本、签名来源和 typed runtime config。`world.toml [[mods]]` 与任意 `[mods.config]` 表不是运行时配置入口，避免玩家可见世界规则文件成为插件供应链控制面。详见 [`specs/core/mod-runtime.md`](../specs/core/mod-runtime.md)。
...
| 操作 | CLI | 效果 |
|------|-----|------|
| 升级 | `swarm mod upgrade <name> <new_version>` | 修改 lock/config/source；rebuild + Engine restart 后生效 |
| 降级 | `swarm mod downgrade <name> <old_version>` | 修改 lock/config/source；rebuild + Engine restart 后切换 |
| 禁用 | `swarm mod disable <name> --world <world>` | 修改 world.toml gating；rebuild + Engine restart 后不注册该 Plugin |
| 卸载 | `swarm mod remove <name>` | 从 ~/.swarm/mods/ 删除，下次编译不再引入 |

Mod 是静态编译的 Bevy Plugin。`add`/`upgrade`/`disable` 只修改 lock/config/source；生效点是 Engine restart 后的 tick boundary，TickInputEnvelope 记录新的 `system_manifest_hash` 与 `mods_lock_hash`。运行中不做 tick 级 hot-swap。
...
struct Resource {
    amounts: BTreeMap<String, u32>,    // 资源键按字节序排序，保证跨平台迭代确定性。advanced/modded example: { "Energy": 500, "Matter": 200 }
}

// Source——可再生资源点
struct Source {
    produces: BTreeMap<String, u32>,   // 资源键按字节序排序。{ "Energy": 1 } or advanced/modded example { "Energy": 1, "Matter": 1 }
    capacity: u32,
...
- **静态出口**：出口默认为固定静态，不可被摧毁或建造。后续可通过模组扩展为可摧毁/可建造出口。
...
阶段一：收集 (COLLECT) — 并行
  ...
  │   ├── Sandbox Container 返回 CommandIntent[]
  │   └── 收集全部指令到指令队列
  └── 详见 specs/core/distributed-sandbox.md
...
阶段二：执行 (EXECUTE) — 约束并行
  ...
  │   └── Spawn 命令在 System Pass 2a 中校验+扣费+写入 PendingEntityCreation
  │   ├── System Pass 2b: ECS Systems
    │   │   └── > **权威系统调度见 [Complete Tick Execution Manifest](../specs/core/phase2b-system-manifest.md)** — 31 systems（System Pass 2a inline 6 + System Pass 2b registered 25），serial spine + 2 parallel sets
  ├── redb 原子提交（全或无）
  └── tick_counter 推进
...
**System Pass 2b 并行策略**：Combat (S11-S13) 按 target_id partition 并行——attack_system、ranged_attack_system、heal_system 操作不重叠的 target 实体。Status Buffer Production (S16-S22b) 按 status subtype 并行——各系统写入互不重叠的 typed buffer（HackBuffer/DrainBuffer/OverloadBuffer/DebilitateBuffer/DisruptBuffer/FortifyBuffer/LeechBuffer/FabricateBuffer），不直接修改 StatusState。status_advance_system (S22) 为串行唯一 StatusState writer——从 S14 读取 canonical sorted pending_intents 并从 S16-S22b 读取 typed buffers，统一推进所有 StatusState。decay (S24) 为独立串行系统（疲劳/冷却/结构衰减不与任何并行系统共享数据）。regeneration (S10) 在 damage_application (S15) 之前串行执行（防止 heal+regen 双倍回复）。完整调度及 R/W 矩阵见 [Complete Tick Execution Manifest](../specs/core/phase2b-system-manifest.md)。
...
3. ECS System 执行顺序固定（见 [Complete Tick Execution Manifest](../specs/core/phase2b-system-manifest.md)，31 systems，
...
**500/1000 player 容量的真正瓶颈**: 不在 worker pool（可水平扩展），而在 (a) Pass 2 sandbox 串行执行时间，(b) Per-player WASM 平均执行时间 × 并行度。详见 §3.4.1 Tick Pipeline 预算与下方容量推导。
...
> **玩家间转账反滥用**：资源转移语义以 `../specs/core/resource-ledger.md` §2.1 为准。不存在新手专用双向转账锁；统一使用 anti-snowball 数学、allied cap、rate limit、intercept 与审计。
...
| **Truncated** | snapshot > cap | 按确定性截断顺序（距离桶 → entity_id 字典序，从最远桶末尾移除）截断；`snapshot.truncated=true`；暴露 `omitted_categories` 和 bucket 统计。**权威截断合同见 [Snapshot Contract](../specs/core/snapshot-contract.md) §1** |
| **Rejected** | 保护性拒绝（如 OOM 攻击） | 返回 deterministic empty input + `over_budget_rejected` 错误码 |

**截断顺序**: 距离桶 0(self) > 1(adjacent) > 2(near) > 3(mid) > 4(far) > 5(very far) > 6(out of sight)，同桶内 entity_id 字典序。关键实体遵循 Snapshot Contract §1.4 的 over-budget degradation：实体本身不完全移除，但可按确定性优先级降级内部字段；若仍超过 256KB，返回 `SnapshotOverBudget` + deterministic minimal snapshot。详见 Snapshot Contract §1.3–1.4。
...
> redb 是嵌入式纯 Rust ACID KV 存储，零外部进程依赖。Docker 镜像 = 单二进制 + 单 `.redb` 文件。存储层不需要分布式事务——引擎是单实例单写者，redb 的 WriteTransaction 原子批量写入完全满足每 tick 原子提交需求。水平分片为未裁决方向，届时再评估存储层是否需要升级。
