...
|------|---------|---------------|---------|
| **System Pass 2a (Inline)** | sorted command loop 串行 inline 应用 | Move, Harvest, ClaimController, Build, Recycle, Transfer, Withdraw, TransferToGlobal, TransferFromGlobal, Spawn (validate + debit + pending), Action dispatch (via ActionRegistry) | **玩家提交的命令**——先将全部命令合并为一个 canonical sorted command list，再逐条执行。效果依赖执行顺序，且「先到先得」竞争有意义。对 Bevy World 做立即修改或产生命令 intent，后续命令基于最新状态校验 |
