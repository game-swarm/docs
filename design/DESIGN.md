...
struct Controller {
    ...
    repair_per_drone: u32,        // 每 drone 回退的 age 量。**硬上限：多个 Controller 的总 age 回退量不超过每 tick 自然增长的 50%（即 max(0, age + 1 - min(0.5, controller_count * 0.5))），不可完全抵消寿命流逝。**
}
```
...
- **动态出入口**：未来 Phase 3+ 考虑可摧毁/可建造的出口（wall breach / tunnel），但 P0 阶段出口为固定静态。
...
Swarm 的扩展分为三个阶段，当前 P0（Phase 1-3）聚焦于垂直扩展：

| 阶段 | 架构 | 玩家容量 | 说明 |
|------|------|---------|------|
| **Phase 1-3（P0）** | 单 Engine 实例（垂直扩展） | 目标 MVP = 500 活跃玩家（硬上限 TBD） | 单机 + FDB 单一提交保证世界一致性 |
| **Phase 4-6** | 单 Engine + FDB 分层缓存 | 1,000-5,000 活跃玩家 | 引入房间级读写分离与区域缓存 |
| **Phase 7+** | 水平分片（多 Engine 实例） | 规模不限 | 跨 Engine 状态同步、跨分片移动为远期关注 |

**P0 范围保证**：FDB（FoundationDB）的单一事务提交保证单世界内的强一致性。水平分片不在 P0 范围内，但数据模型和 API 设计预留了分片扩展的接口（如房间级 shard key 已在数据模型中预留）。
...
| `active_aging` | — | 110% | 每 tick 执行命令的 drone 额外 +10% 衰老，防止挂机囤兵 |
...
与资源类型一样，建筑类型可通过 world.toml 定义。默认世界提供以下 12 种基础类型：
...
| **特殊攻击** | 全部 6 种可用：`Hack`, `Drain`, `Overload`, `Debilitate`, `Disrupt`, `Fortify` | 冷却时间与资源消耗见 §8 特殊攻击方式表格 |
...
| 确定性节点预算 | 100,000 AST 节点/tick | 强制终止当前模组，**该模组本 tick 的所有 actions 全部回滚**（事务性隔离）。超限模组记录警告，连续 10 tick 超限自动禁用。AST 节点数是确定性度量——同一输入在任何硬件上终止于相同节点，保证 state_checksum 可复现。墙钟仅用于告警监控（如单模组 >2s 触发运维告警），不作为状态决定因素。 |
