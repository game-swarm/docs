...
struct Controller {
    ...
    repair_per_drone: u32,        // 每 drone 回退的 age 量
}
```
...
| 3 | 500 | Extension (10), Tower, Storage, Depot | 200 | 20/tick | 2 格 | 防御+前线维修 |
| 4 | 1,500 | Extension (20), Link | 300 | 30/tick | 2 格 | 能源网络 |
| 5 | 5,000 | Extension (30), Terminal, Observer | 400 | 40/tick | 3 格 | 市场交易 |
| 6 | 15,000 | Extension (40), Extractor, Lab, Factory | 500 | 50/tick | 3 格 | 制造系统 |
| 7 | 50,000 | Extension (50), PowerSpawn | 500 | 60/tick | 4 格 | 晚期产能 |
| 8 | 150,000 | Extension (60), Nuker | 500 | 80/tick | 5 格 | 终极武器 |
...
**Age 恢复**: drone 必须**移动到 Controller 或 Forward Depot 维修范围内**才能降低 age。Controller 维修距离随 RCL 增长（RCL1=1 格，RCL8=5 格），免费，每 tick 服务上限由 RCL 决定。Forward Depot 固定 range=1，消耗存储资源。相邻格只有 6 个——大量 drone 需要排队，形成物流拥挤决策。Healer body part 只能恢复 HP，不能降低 age。
