...
| 规则 | 类型 | 默认 | 说明 |
|------|------|------|------|
| `drone_lifespan` | u32 | 1500 | drone 最大存活 tick 数。达到后自动死亡（`death_system` 回收）。**续期**: 玩家拥有的每个 Controller 每 tick 给全局所有 drone 回退 age 0.5 tick（多 Controller 可叠加，上限为完全抵消自然 age 增长）。不再依赖一次性占领事件。**冷却**: 无——改为持续维持模型，消除"占领-放弃-再占领"的 farming 策略 |
