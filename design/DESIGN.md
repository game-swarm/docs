...
/// 默认 1500 tick（≈75 分钟 @ 3s/tick），可通过 world.toml 配置。
...
| 规则 | 类型 | 默认 | 说明 |
|------|------|------|------|
| `drone_lifespan` | u32 | 1500 | drone 最大存活 tick 数。达到后自动死亡（`death_system` 回收）。1500 tick ≈ 75 分钟 @ 3s/tick。**续期机制**: 占领新 Controller 房间时，该玩家全部 drone 的 age 重置为 0——鼓励扩张而非龟缩。**冷却**: 同一玩家 500 tick 内只能触发一次续期（防 ping-pong 滥用） |
...
|---------|--------------|------|------|---------|------|
| **Hack** | Claim | 尝试夺取目标 drone 控制权。成功条件：`目标剩余 hits < 最大 hits × hack_threshold`（默认 0.3） | 100 tick | 500 Energy | 目标 `Psionic` 抗性 |
| **Drain** | Carry + Work | 从目标建筑/存储中窃取资源，每 tick 转移 `carry_capacity` 单位 | 50 tick | 200 Energy/tick | 目标 `EMP` 抗性 |
| **Overload** | RangedAttack | 消耗目标计算配额。目标 `fuel budget` 减少 500k（默认 MAX_FUEL=10M 的 5%） | 200 tick | 300 Energy | 目标 `EMP` 抗性 |
...
| 墙钟执行时间 | 100ms/tick | 强制终止，模组标记为 "degraded" |
