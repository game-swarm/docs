...
- 100 个 u32 变量 = 400 bytes，10 个 i64 = 80 bytes——如果世界配置了 `memory_spawn_cost = { Energy = 0.5 }`，前者孵化成本 200 Energy，后者 40 Energy
- 如果配置了 `memory_upkeep_cost = { Energy = 0.01 }`，前者每 tick 消耗 4 Energy，后者 0.8 Energy
- 这创造了策略深度：玩家需要权衡「多存状态方便决策」和「内存开销」
- 默认免费——简单世界不强制经济复杂度
