...
| **单层 CA** | 不分 Root-CA/issuing-CA 层级——单服务器部署双层无安全收益。Server CA 密钥对直接签发证书 |
...
不分 Root-CA/issuing-CA 层级——双层签名链在单服务器部署中没有安全收益。攻击者能拿到 issuing CA 私钥就能拿到 Root CA 私钥（在同一台机器上）。
...
| `CodeSigningCertificate` | WASM/module deploy 签名 | 30-180d（默认 30d，world.toml 可配） | 只能签 `SWARM-DEPLOY-V1` 载荷 |

Admin 操作 = `ClientAuthCertificate` + admin scope flag（不需要独立证书类型）。Identity mapping 是身份映射协议，不需要独立证书类型。
