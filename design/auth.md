...
**注意**：请求中**不包含** `challenge`、`difficulty` 或 `email` 字段 — 服务端从 FDB 读取 challenge 权威值；邮箱绑定必须在证书签发后通过已认证的 `swarm_bind_email` 完成。CSR 内的 public key 必须验证 `csr_signature`。
...
Deploy 使用 FDB 持久化的 `version_counter`，且 `swarm_deploy` 采用同步载荷：RPC 请求直接携带 `wasm_bytes`、`metadata` 与签名 `DeployPayload`，服务端在同一请求内完成验证、hash 计算、编译准备与 FDB manifest commit。对象存储只用于 commit 后持久化原始 WASM blob，不参与 deploy 是否被接受的判定：
...
swarm_bind_email(email)
  ...
  → 发送验证邮件
  → 验证通过后 FDB auth/users/<username>.email_verified = true
```
