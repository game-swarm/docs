...
- 审计：每次签发写入 redb `auth/cert_audit/<certificate_id>`，记录 `player_id, public_key_id, usage, issued_at, issuer, scopes, ttl`
...
- Server Intermediate CA 私钥存储要求：
  ...
  **Intermediate CA 私钥保护强制要求**：
  ...
  - **审计合同**：每次 Intermediate CA 私钥使用（签发证书）写入 redb `auth/ca_audit`，记录 `timestamp, certificate_id, issuer_id`。私钥轮换事件写入 `auth/ca_rotation_log`。
...
| `deploy_mutation` | 部署请求——防重放由 redb version_counter 保证 | **redb version_counter**（见 §10.8） | `swarm_deploy` |
| `non_idempotent_mutation` | 重复执行产生副作用 | redb WriteTransaction 内消费 challenge 或 version counter | `swarm_submit_csr`（redb WriteTransaction 内消费 PoW challenge，一次性）、`swarm_admin_set_world_config` |
| `admin_critical` | 安全敏感管理操作 | redb WriteTransaction 内消费 challenge + 双签审计 | `swarm_revoke_certificate`、CA 操作 |

Dragonfly nonce 仅用于 `read_replay_safe` 和 `idempotent_mutation`。所有 `non_idempotent_mutation` 和 `admin_critical` 操作必须使用 redb version counter、idempotency key 或一次性 challenge，并在事务内消费。
...
**事务约束**：
- 密码哈希在事务外完成（argon2id ~100ms），redb WriteTransaction 保持短生命周期（<10ms）
- 事务冲突重试最多 3 次，返回 `McpError::conflict_retry`
- redb key 使用 `auth/` 前缀隔离游戏世界状态
...
- `identity_fingerprint` = 完整 `blake3("local:" + login_username_lowercase)` 或 `blake3("federated:" + world_id + ":" + original_player_id)` 的 **全部 32 字节**，不经 64-bit 截断
  ...
  - **redb 存储**：在 `auth/users/<login_username>` 中作为可选字段；在 `auth/identities/` 和跨世界联邦握手消息中包含
...
密码哈希在事务外执行（避免 redb WriteTransaction 内高延迟）。
...
fn generate_challenge(difficulty_bits: u8) -> PoWChallenge {
    PoWChallenge {
        ...
        difficulty_bits,       // 服务端权威值，存储于 redb
...
**服务端验证流程**（redb WriteTransaction 内）：

```rust
// 1. 从 redb 读取 challenge（服务端权威）
...
//    实现: 在 PoW 验证之前或之后分支，但整个流程在同一 redb WriteTransaction 内
```
...
| `swarm_submit_csr` | `username`, `csr`, `certificate_profile`, `device_label?`, `challenge_id`, `nonce`, `csr_signature`, `recovery_password?` | `CertificateBundle` | 完成 PoW + CSR 验证 + 证书签发；不接受 `email`，邮箱绑定必须通过 `swarm_bind_email` 完成 |
...
**注意**：请求中**不包含** `challenge`、`difficulty` 或 `email` 字段 — 服务端从 redb 读取 challenge 权威值；邮箱绑定必须在证书签发后通过已认证的 `swarm_bind_email` 完成。CSR 内的 public key 必须验证 `csr_signature`。
...
请求 nonce 防重放不写 redb。使用 Dragonfly SETNX TTL 作为热路径权威存储：
...
高价值操作（admin 证书签发、恢复流程、Admin MCP 工具）使用 challenge-response nonce，nonce 消费在 **redb WriteTransaction 内原子执行**：服务端维护 per-admin 单调递增计数器 `auth/admin_nonce_counter/<admin_id>`，每个 admin challenge 绑定当前计数器值。消费时在 redb WriteTransaction 内执行原子 CAS（compare-and-swap）递增计数器——若计数器已被递增（重放）则事务冲突回滚，拒绝操作。此机制保证 admin 操作的严格一次性语义，崩溃后不重放。
...
| Deploy 请求 | Version Counter (redb) | redb `version/{player_id}` | 严格递增，崩溃后不重放 |
| Admin 高权限操作 | redb monotonic counter (CAS atomic) | redb `auth/admin_nonce_counter/<admin_id>` | 严格递增，崩溃后不重放 |
| CSR 提交 | PoW nonce | redb `challenge/{id}`，consumed 标记 | 一次性消费，原子标记 |

Deploy 使用 redb 持久化的 `version_counter`，且 `swarm_deploy` 采用同步载荷：RPC 请求直接携带 `wasm_bytes`、`metadata` 与签名 `DeployPayload`，服务端在同一请求内完成验证、hash 计算、编译准备与 redb manifest commit。对象存储只用于 commit 后持久化原始 WASM blob，不参与 deploy 是否被接受的判定：
...
swarm_deploy:
  redb WriteTransaction:
    ...
    write deploy/{player_id}/{current + 1} = {wasm_module_hash, compiled_artifact_hash, ...}
```

同一 player 的 deploy 严格按 version_counter 顺序执行。即使 Dragonfly 崩溃，deploy 也不重放——redb WriteTransaction 保证 version_counter 原子递增。

#### Audience 字符串语法

所有 audience 字符串使用规范语法：`swarm-aud-v1:<transport>:<server_id>:<world_id>:<subject_id>`

- `transport`：客户端传输类型，枚举：`browser-http` | `agent-mcp` | `cli-rest` | `agent-ws` | `spectator-ws` | `replay-viewer`
...
| 证书吊销状态 (CRL) | redb | Engine 内 LRU | World 默认 60s（持久世界容忍度更高）；Arena/competitive 默认 5s（吊销后至多 5s 旧证书仍可被接受） |
| 证书链验证结果 | — | Engine 内 LRU | 0（即时失效） |
| Server Root CA | redb | Engine 启动时加载 | 0（重启生效） |
| Intermediate CA | redb | Engine 启动时加载 | 0（重启生效） |
| 用户 session | Dragonfly (TTL) | 无 | 0 |
| 账号状态（锁定/删除） | redb | Dragonfly (30s TTL) | 30s |
...
| 恢复凭据校验 | 10/min per IP + 5 次失败锁 5min per username | Dragonfly + redb |
...
- bump 事件写入 redb `auth/epoch_history`，记录 reason + timestamp + admin_id
...
恢复确认流程必须携带 `recovery_reason`，服务端在同一 redb WriteTransaction 内签发新证书、吊销所有现有 refresh token，并按下表执行旧证书处理策略。客户端不得用“用户选择”覆盖安全默认；用户界面只能在允许保留的场景中提供后续手动吊销入口。
...
- 验证旧密码 → argon2id hash 新密码 → 更新 redb
...
- 验证 token：未过期、未使用、匹配 redb `auth/reset/` 记录
- 验证 `new_csr` 签名与 requested profile/scope，签发新的应用层证书 bundle
- 若提供 `new_password`：argon2id hash 新 recovery password → 更新 redb
...
- 用户点击链接验证后，redb `auth/users/<username>` 更新 `email_verified` + `email`
...
```
swarm_bind_email(email)
  → 需要有效 ClientAuthCertificate 或 Web session 兼容 token
  → 发送验证邮件
  → 验证通过后 redb auth/users/<username>.email_verified = true
```
...
```
1. 标记 redb auth/users/<username>.deleted_at = now
...
| 30 天后 | `deleted_at` 满 30 天 → 永久清除 redb 记录 + 释放 username |
...
| `refresh_token` | 30 days | 每次使用后轮换 | redb `auth/sessions/`（Web 兼容层） |
...
grace 使用必须原子消费：redb 中设置 grace_consumed_at，避免重复使用
...
| `refresh_token` | Web session 兼容续签材料 | 服务端随机生成，redb 存储，每次使用后旋转 |
...
| PoW challenge 替换/降级 | 服务端从 redb 读取权威 challenge+difficulty，register 不接收客户端 challenge 字段 |
| PoW challenge 重放 | 一次性消费（redb 原子标记 consumed），TTL 5min |
| PoW challenge DoS | challenge 申请 IP 限速 10/min；redb 存储 TTL 自动清理 |
...
| redb 泄露 | recovery password 仅存储 argon2id hash；所有 value 带 schema_version |
...
| 组件 | 文件 | 变更 |
|------|------|------|
| Auth Service | `src/auth/mod.rs` (新) | challenge 生成/验证、CSR 审核、证书签发/续签/吊销、恢复流程、账号删除、redb 读写 |
