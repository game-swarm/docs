...
| 7. 首个 PvE 挑战 | 中立 NPC drone 出现在视野 → 编写攻击/规避逻辑 → 完成首次 PvE 击杀 | `pve-spawning` 结算配置的 PvE award 事件 | 2-3min |
...
4. 使用 CodeSigningCertificate 提交 `{wasm_bytes, mod_toml, DeployPayload, code_signature, certificate_id}` 到 tutorial world（swarm_deploy → 等待 first_tick_executed）
```

Starter bot 代码中的字段名（`sequence`, `Spawn`, `Move`, `Harvest` 等）必须遵循 IDL 契约。Engine 的教程合同测试检查已发布示例；docs-side `scripts/sync_api_registry.py --check` 覆盖 Registry metadata/count/version drift，但不生成 starter bot 源码。
...
- 实体字段自动补全（drone.fatigue, source.resources["Energy"] 等）
...
- 赛后生成回放（房间公开则回放公开）
- 无自动匹配、无天梯排名、无赛季
- Tournament/League 是上层编排扩展，通过多场 Room Match 组合实现，不改变单场 Room Match 的核心语义
