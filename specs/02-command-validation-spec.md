...
| `source` | Source | **服务端注入** | 见 specs/09-command-source-model §2.1 来源矩阵 |
...
> - **Admin 路径统一**：Admin 命令走同一 `validate_and_apply()` 管线，仅 RejectionReason 阈值放宽；编译期通过 trait 设计确保无独立代码路径可绕过（见 specs/09-command-source-model §2.3）
...
锚定 DESIGN.md §5, §8.2。以下 CommandAction 变体。
