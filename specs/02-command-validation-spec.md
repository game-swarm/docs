...
Drone 在 Phase 2b spawn_system 中创建——位于 death_mark（释放 room cap 槽位）之后、combat/decay/death_cleanup 之前。新 drone 在同 tick 参与战斗和衰减——可能出生即被攻击或受衰减影响（有意设计，非文档错误）。
...
**注意**: §3.12-3.17 特殊攻击实现完毕。IDL 定义见 P0-8。
...
| **MoveTo** | `object_id.owner == player_id` | 路径长 ≤ MAX_PATH_LENGTH (100) | N/A | N/A | 目标 (x,y) 在房间内 | 同 Move + 路径存在性, `body.move_parts ≥ path_len` |
...
| **Repair** | `object_id.owner == player_id` | `object_id` 距 `target_id` ≤ 3 | N/A | `drone.carry[Energy] ≥ repair_cost` | N/A | `drone.body` 含 `Work`+`Carry`, `target.hits < hits_max`, target 是 Structure |
...
| **Overload** | `object_id.owner == player_id` | N/A (逻辑攻击，无 range 限制) | N/A | Energy ≥ 300 (消耗) | N/A | `drone.body` 含 `RangedAttack`, target 非己方, `target.fuel_budget > MAX_FUEL×0.2`, `fatigue==0`, 冷却未到 |
...
> - **Admin 路径统一**：Admin 命令走同一 `validate_and_apply()` 管线，仅 RejectionReason 阈值放宽；编译期通过 trait 设计确保无独立代码路径可绕过（见 P0-9 §2.3）
...
## 8. 新增 CommandAction 变体

锚定 DESIGN.md §5, §8.2。以下 CommandAction 新增。
