...
| 7 | **AlliedTransfer** | transfer | TransferToGlobal + TransferFromGlobal (allied only) | Allied transfer with 200 bp (2.00%) fee. 200 tick delay. 500 tick cooldown per receiver. Daily cap: `max(10_000, receiver_gcl × 20_000) × allied_daily_cap_world_multiplier / 100` units per receiver. Both players must share active alliance ≥ 100 tick. Per `specs/core/08-resource-ledger.md` §2.1. |
