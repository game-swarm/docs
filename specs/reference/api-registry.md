...
| 5 | `Action` | `action_type: string, target_id?: EntityId, payload: ActionPayload` | core | Execute an ActionRegistry action; custom-action wire type is `action_type` |
...
| # | Action | 参数 | 分类 | 说明 |
|---|--------|------|------|------|
| 5 | `Action` | `action_type: string, object_id: EntityId, target_id?: EntityId, payload: ActionPayload` | action | 执行 ActionRegistry 注册的 combat/effect action；wire `type` 为 action_type 值 |
