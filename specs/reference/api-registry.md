...
| auth_api version | `0.2.0` | auth_api.idl.yaml |
...
| MCP tools all_declared | 58 | game_api.idl.yaml + auth_api.idl.yaml |
| MCP tools active_only | 57 | game_api.idl.yaml + auth_api.idl.yaml |
| MCP tools gated | 1 | game_api.idl.yaml |
| Host functions | 7 | game_api.idl.yaml |
| Economy operations | 7 | economy.idl.yaml |
...
Mod 扩展通过 `world.toml [[custom_actions]]` 注册非保留名称的 combat/effect action。TickTrace 记录 `world_action_manifest_hash`，确保 ActionRegistry 变更进入 replay 确定性边界。
...
#### Arena (`all_declared=4`, `runtime_active=4`, `implementation_feature_gated=0`)
...
| `swarm_tournament_precommit` | `{module_id}` | `{status, locked_module}` | 5/tick | `swarm:deploy` | `player_id` | `idempotent_mutation` | `owner` | `per_player` | game_api |
| `swarm_tournament_status` | `{}` | `{tick, player_id, mode, deploy_locked, locked_module?, preparation_tools, direct_gameplay_tools_enabled, tournaments}` | 50/tick | `swarm:read` | `world` | `read_replay_safe` | `none` | `global` | game_api |
