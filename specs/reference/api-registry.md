...
> **本文档是 Swarm API 的设计时 canonical schema authority。** `game_api.idl.yaml`、`auth_api.idl.yaml`、`economy.idl.yaml` 是实现侧代码生成输入，必须由 CI 与本文档逐字段校验一致；出现冲突时修改产生分歧的源文件并同步再生成，不允许让实现 artifact 覆盖设计权威。
...
**API 版本**: `0.5.0` (game_api) / `0.1.0` (auth_api) / `0.1.1` (economy)
**Schema inputs**: [game_api.idl.yaml](game_api.idl.yaml), [auth_api.idl.yaml](auth_api.idl.yaml), [economy.idl.yaml](economy.idl.yaml)
**Codegen**: [codegen.md](codegen.md) — Registry/IDL → SDK 生成链，禁止手写分叉。

## 原则

1. **单事实源**：Registry 是设计时 canonical schema authority；YAML IDL 是机器可读实现 artifact，必须与 Registry 同步。
...
Core CommandAction 是 WASM tick() 输出的 CommandIntent 中的 action type，包含 10 种非战斗基础操作 + 1 个 Action dispatch，共 11 种 CommandAction 变体。Combat/effect action 不再作为独立 CommandAction 变体注册，而是通过 `Action` dispatch 进入 ActionRegistry。
...
**变体总数**: 10 个非战斗基础操作 + 1 个 Action dispatch = 11 个 CommandAction 变体；ActionRegistry 是 dispatch 目标注册表，不增加 CommandAction enum 变体数。
...
所有拒绝原因在此注册。覆盖 Pipeline、Validation、MCP、Runtime、Auth 五层。canonical wire code 总数为 48。

> **CI Gate**: 本计数为 canonical 48。若与其他文档或 IDL 不一致，应同步 Registry 与 IDL YAML，CI 检查模式拒绝二者分叉。
...
| 40 | `DeviceNotRegistered` | Device is not registered in the device trust registry | auth_api | |
| 41 | `SessionLimitReached` | Player has reached the maximum number of concurrent sessions | auth_api | |
| 42 | `RefreshTokenInvalid` | Refresh token is invalid, expired, or already consumed (replay detected) | auth_api | Triggers token family revocation |
| 43 | `ScopeInsufficient` | Token scope does not include the required permission | auth_api | |
| 44 | `TokenRevoked` | Access or refresh token has been explicitly revoked | auth_api | |
| 45 | `MultiDeviceConflict` | Device registration or session creation conflicts with existing device policy | auth_api | |
| 46 | `UnknownCredential` | Credential type is not recognized or not supported | auth_api | |
| 47 | `InternalAuthError` | Internal auth subsystem error | auth_api | E.g., token store unavailable, CA unreachable |
| 48 | `NotEligible` | Target is not eligible for the requested action or is gated by rule state | game_api | Generic action eligibility code |
...
| 7 | `host_get_fuel_remaining` | `() -> u64` | ✅ | Remaining WASM fuel budget for the current tick |
...
Algorithm: Ed25519. Validation: single application-layer certificate signed by Server CA → public key → signature → timestamp window (60s) → nonce → scope/audience. `certificate_id` is read from the certificate body, not from a request header.
...
| 2 | **StorageTax** | taxation | Every tick, per player | Percentage-based tiered tax on global storage. Tiers: 0%–30% cap = 0 bp, 30%–60% = 1 bp, 60%–85% = 5 bp, 85%–100% = 20 bp. Per `specs/core/resource-ledger.md` §2.2 authoritative tiered formula. Min 1 unit if rate > 0. |
| 3 | **UpkeepDeduction** | maintenance | Every tick, per player | Empire-wide superlinear upkeep: `base_upkeep × rooms × (1 + rooms / room_soft_cap)`. Deducted from global storage. Standard defaults: base_upkeep=50, room_soft_cap=10. Per `specs/core/resource-ledger.md` §Empire Upkeep. |
...
Persistence 层处理非 deploy 的大型二进制对象存储（如 replay recording、snapshot archive）。Per D5/B + R22 B1：redb WriteTransaction 原子提交 **replay-critical subset**（10 项必填字段，见 `persistence-contract.md` §2.1）；辅助 object-store blob 上传可异步执行并通过 manifest 记录审计状态。Deploy payload 不走本节异步上传路径。
...
| 0.1.0 | 2026-06-18 | game_api | R15 B1+B4+H1: 初次建立权威注册表。统一 RejectionReason、CommandAction、MCP Tools、Host Functions、容量限制、TickTrace Envelope、Direction4、错误格式。 |
