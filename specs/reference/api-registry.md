...
| 工具名 | Input Schema | Output Schema | Rate Limit | Required Scope | Subject Source | Replay Class | Visibility Filter | Rate Limit Key | 来源 IDL |
|--------|-------------|---------------|------------|----------------|----------------|--------------|-------------------|----------------|----------|
| `swarm_deploy` | `{player_id, drone_id, deploy_payload, code_signature, certificate_id, version_counter, metadata}` | `{deploy_id, accepted, validation_errors, fdb_version_counter}` | 10/h | `swarm:deploy` | `player_id` | `deploy_mutation` | `owner` | `per_player` | game_api |
...
| `swarm_get_deploy_status` | `{deploy_id}` | `{status, errors, deployed_at, fdb_version_counter}` | 20/tick | `swarm:read` | `player_id` | `read_replay_safe` | `owner` | `per_player` | game_api |
...
> **swarm_deploy** 使用 `deploy_mutation` replay class (R16 B6, R22 B1, D4 统一): Deploy 为同步提交。请求内的 `deploy_payload`、`code_signature`、`certificate_id`、`version_counter` 一并进入 FDB 原子事务；服务端验证签名、证书与版本计数器后，同步写入 deploy manifest 并返回 `fdb_version_counter`。Deploy 完整状态机见 `specs/core/05-persistence-contract.md` §2.3。Replay verifier 以 `fdb_version_counter` 全序重放，replay-critical 字段清单见 `05-persistence-contract.md` §2.1。
...
Deploy 子系统使用 deploy_mutation 模式：`swarm_deploy` 同步提交 `deploy_payload`、`code_signature`、`certificate_id`、`version_counter` 与 metadata。FDB 在单个原子事务中验证并写入 deploy manifest，同时递增并返回 `fdb_version_counter`。部署提交不依赖异步 blob 上传路径；replay 以 FDB manifest 与 `fdb_version_counter` 为权威。
...
| 3. Commit Manifest | FDB 事务提交 manifest record: `{deploy_id, drone_id, deploy_payload_hash, certificate_id, version_counter, metadata, fdb_version_counter}` | engine |
...
Persistence 层处理非 deploy 的大型二进制对象存储（如 replay recording、snapshot archive）。Per D5/B + R22 B1：FDB 事务原子提交 **replay-critical subset**（10 项必填字段，见 `05-persistence-contract.md` §2.1）；辅助 object-store blob 上传可异步执行并通过 manifest 记录审计状态。Deploy payload 不走本节异步上传路径。
