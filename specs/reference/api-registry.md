...
> **共享字段 `object_id: EntityId`**: 所有 21 个 CommandAction 变体均包含 `object_id`（执行该动作的 entity）。此字段为所有 action 的公共参数，不在下方表格中各 action 的"参数"列重复列出。IDL 中每个 action 的 schema 均包含 `object_id`。
...
```json
{
  ...
  "error": {
    ...
    "data": {
      "rejection_reason": "RejectionReason (canonical enum, e.g. InsufficientResource)",
