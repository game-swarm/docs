...
所有 MCP 工具在此注册。**共计 54 个活跃工具** (game_api) + **11 个 Auth API 工具** (auth_api)。

**来源 IDL**: game_api.idl.yaml (54 tools), auth_api.idl.yaml (11 auth tools)
...
| 工具名 | Input Schema | Output Schema | Rate Limit | Required Scope | Subject Source | Replay Class | Visibility Filter | Rate Limit Key | 来源 IDL |
|--------|-------------|---------------|------------|----------------|----------------|--------------|-------------------|----------------|----------|
| `swarm_get_leaderboard` | `{scope, limit}` | `{entries: [{player, gcl, rooms, drones}]}` | 5/tick | `swarm:read` | `world` | `read_replay_safe` | `none` | `global` | game_api |
