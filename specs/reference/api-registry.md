...
## 2. RejectionReason — 46 codes

所有拒绝原因在此注册。覆盖 Pipeline、Validation、MCP、Runtime、Auth 五层。canonical wire code 总数为 46。

> **CI Gate**: 本计数为 canonical 46。若与其他文档或 IDL 不一致，应同步 Registry 与 IDL YAML，CI 检查模式拒绝二者分叉。
...
**来源 IDL**: game_api.idl.yaml (`all_declared=57`, `active_only=53`, `rfc_gated=4`), auth_api.idl.yaml (`all_declared=7`, `active_only=7`, `rfc_gated=0` — 证书模型)
