...
| 工具名 | Input Schema | Output Schema | Rate Limit | Required Scope | Subject Source | Replay Class | Visibility Filter | Rate Limit Key | 来源 IDL |
|--------|-------------|---------------|------------|----------------|----------------|--------------|-------------------|----------------|----------|
| `swarm_auth_login` | `{credential, challenge_response}` | `{token, expiry, scope, player_id}` | 10/min | `swarm:auth` | `none` | `non_replayable` | `none` | `per_ip` | game_api |
| `swarm_auth_refresh` | `{token}` | `{token, expiry}` | 5/min | `swarm:auth` | `player_id` | `non_replayable` | `owner` | `per_player` | game_api |

> **Note**: `swarm_auth_login` 和 `swarm_auth_refresh` 在 auth_api.idl.yaml 中有更丰富的 schema（含 `device_id`, `device_name`, `refresh_token`, `session_id` 等字段）。game_api 版本为其简化形态。完整 schema 见 §3.4 Auth API 工具。
...
> **R27 ML-9**: `swarm_auth_login` 和 `swarm_auth_refresh` 在 game_api（§3.2）中为简化形态——此处为完整 schema。game_api 中的版本标注 `schema_source=auth_api`，SDK 不生成重复函数。`swarm_auth_check` 同样存在简化（game_api §3.2）→ 完整（auth_api §3.3）两个版本。game_api 版本的 response 字段集是 auth_api 的子集。
