...
| 10 | `Recycle` | `target_id: EntityId` | core | Recycle a drone or structure |
...
| 16 | `Overload` | `target_id: EntityId` | special_attack | Reduce target fuel budget |
...
所有指令拒绝原因在此注册。**共计 47 个 canonical code**（35 from game_api + 12 from auth_api），覆盖 Pipeline、Validation、MCP、Runtime、Auth 五层。
...
**来源 IDL**: auth_api
**总 canonical codes**: 12 (indices 1001–1012)
...
所有 MCP 工具在此注册。**共计 57 个活跃工具** (game_api) + **11 个 Auth API 工具** (auth_api)。

> ⚠️ **CI Gate**: 工具计数由 IDL YAML 自动生成。若与其他文档或 IDL 不一致，**禁止手工修改此数**——应修正 IDL YAML 源并重新运行 `generate_api_registry.py`。CI 检查模式拒绝手工编辑导致的不一致。

**来源 IDL**: game_api.idl.yaml (57 tools), auth_api.idl.yaml (11 auth tools)
...
| `swarm_get_snapshot` | `{player_id}` | `{tick, entities, terrain, resources, truncated, omitted_count}` | 1/tick | `swarm:read` | `player_id` | `read_replay_safe` | `fog_of_war` | `per_player` | game_api |
...
|--------|-------------|---------------|------------|----------------|----------------|--------------|-------------------|----------------|----------|---------------|----------|
| `swarm_auth_login` | `{credential, challenge_response}` | `{token, expiry, scope, player_id}` | 10/min | `swarm:auth` | `none` | `non_replayable` | `none` | `per_ip` | game_api | `auth_api` | `auth_api.swarm_auth_login` |
| `swarm_auth_refresh` | `{token}` | `{token, expiry}` | 5/min | `swarm:auth` | `player_id` | `non_replayable` | `owner` | `per_player` | game_api | `auth_api` | `auth_api.swarm_auth_refresh` |
| `swarm_auth_check` | `{token}` | `{valid, player_id, scope, expiry}` | 100/min | `swarm:auth` | `player_id` | `read_replay_safe` | `owner` | `per_player` | game_api | `auth_api` | `auth_api.swarm_auth_check` |

> **R27 ML-9 机器可读**: `schema_source=auth_api` + `alias_of=<auth_api tool>` 使 codegen 可自动跳过这些 game_api auth shortcuts，从 `auth_api.idl.yaml` canonical source 生成唯一实现。`swarm_auth_check` 在 game_api 中为简化 response（`valid, player_id, scope, expiry`），完整 response（含 `session_id, device_id`）见 §3.3 auth_api。
...
| 工具名 | Input Schema | Output Schema | Rate Limit | Required Scope | Subject Source | Replay Class | Visibility Filter | Rate Limit Key | 来源 IDL |
|--------|-------------|---------------|------------|----------------|----------------|--------------|-------------------|----------------|----------|
| `swarm_deploy` | `{player_id, drone_id, wasm_bytes, metadata}` | `{deploy_id, accepted, validation_errors, fdb_version_counter, object_store_key}` | 10/h | `swarm:deploy` | `player_id` | `deploy_mutation` | `owner` | `per_player` | game_api |
...
| `swarm_get_deploy_status` | `{deploy_id}` | `{status, errors, deployed_at, fdb_version_counter, object_store_key}` | 20/tick | `swarm:read` | `player_id` | `read_replay_safe` | `owner` | `per_player` | game_api |
| `swarm_list_deployments` | `{player_id}` | `{deployments: [{id, drone_id, status, at, fdb_version_counter}]}` | 20/tick | `swarm:read` | `player_id` | `read_replay_safe` | `owner` | `per_player` | game_api |
...
> **swarm_deploy** 使用 `deploy_mutation` replay class (R16 B6, R22 B1 统一): WASM blob 异步上传至 object store。FDB 原子提交 deploy manifest + hash pointer + `fdb_version_counter`。Deploy 完整状态机见 `specs/core/05-persistence-contract.md` §2.3。Replay verifier 以 `fdb_version_counter` 全序重放，不依赖对象存储 blob 可用性。replay-critical 字段清单见 `05-persistence-contract.md` §2.1。
...
| 工具名 | Input Schema | Output Schema | Rate Limit | Required Scope | Subject Source | Replay Class | Visibility Filter | Rate Limit Key | 来源 IDL |
|--------|-------------|---------------|------------|----------------|----------------|--------------|-------------------|----------------|----------|
| `swarm_get_leaderboard` | `{scope, limit}` | `{entries: [{player, gcl, rooms, drones}]}` | 5/tick | `swarm:read` | `world` | `read_replay_safe` | `arena_only` | `global` | game_api |
| `swarm_tournament_create` | `{name, rules, start_at?}` | `{tournament_id, status}` | 10/h | `swarm:admin` | `admin_id` | `admin_critical` | `admin_scope` | `per_admin` | game_api |
| `swarm_tournament_precommit` | `{tournament_id, player_id, wasm_hash}` | `{accepted, precommit_id}` | 1/tick | `swarm:deploy` | `player_id` | `idempotent_mutation` | `owner` | `per_player` | game_api |
| `swarm_tournament_status` | `{tournament_id}` | `{tournament_id, status, players, tick, brackets}` | 10/tick | `swarm:read` | `world` | `read_replay_safe` | `none` | `global` | game_api |
...
> **R27 ML-9 机器可读**: game_api §3.2 Auth 表中的 `schema_source` 和 `alias_of` 列使 codegen 自动跳过这些 auth shortcuts，从 `auth_api.idl.yaml` canonical source 生成唯一实现。`swarm_auth_check` 在 game_api 中为简化 response（`valid, player_id, scope, expiry`），完整 response（含 `session_id, device_id`）见本表 Lifecycle Tools。

**来源 IDL**: auth_api.idl.yaml

Auth API 提供 5 个 session lifecycle 工具 + 6 个证书管理和设备注册工具，总计 11 个。其中 `swarm_auth_login` 和 `swarm_auth_refresh` 与 game_api Auth category 重叠，此处为完整 schema。

#### Lifecycle Tools (5)

| 工具名 | Input Schema | Output Schema | Rate Limit | Required Scope | Subject Source | Replay Class | Visibility Filter | Rate Limit Key |
|--------|-------------|---------------|------------|----------------|----------------|--------------|-------------------|----------------|
| `swarm_auth_login` | `{credential, challenge_response, device_id, device_name}` | `{token, refresh_token, expiry, scope, player_id, session_id}` | 10/min | `swarm:auth` | `none` | `non_replayable` | `none` | `per_ip` |
| `swarm_auth_refresh` | `{refresh_token}` | `{token, expiry}` | 5/min | `swarm:auth` | `player_id` | `non_replayable` | `owner` | `per_player` |
| `swarm_auth_logout` | `{token, session_id}` | `{revoked, affected_sessions}` | 20/min | `swarm:auth` | `player_id` | `non_replayable` | `owner` | `per_player` |
| `swarm_auth_check` | `{token}` | `{valid, player_id, scope, expiry, session_id, device_id}` | 100/min | `swarm:auth` | `player_id` | `read_replay_safe` | `owner` | `per_player` |
| `swarm_auth_revoke` | `{target_player_id, target_session_id, reason}` | `{revoked, affected_sessions, audit_log_id}` | 5/min | `swarm:admin` | `admin_id` | `admin_critical` | `admin_scope` | `per_admin` |

#### Cert Management & Device Registration Tools (6)

| 工具名 | Input Schema | Output Schema | Rate Limit | Required Scope | Subject Source | Replay Class | Visibility Filter | Rate Limit Key |
|--------|-------------|---------------|------------|----------------|----------------|--------------|-------------------|----------------|
| `swarm_auth_cert_rotate` | `{cert_id, reason}` | `{new_cert_id, new_cert_fingerprint, old_cert_revoked, rotation_time, expiry}` | 10/h | `swarm:admin` | `admin_id` | `admin_critical` | `admin_scope` | `per_admin` |
| `swarm_auth_cert_issue` | `{subject, san_dns_names, san_ip_addresses, validity_days, key_type}` | `{cert_id, certificate, fingerprint, issued_at, expiry}` | 10/h | `swarm:admin` | `admin_id` | `admin_critical` | `admin_scope` | `per_admin` |
| `swarm_auth_cert_revoke` | `{cert_id, reason}` | `{revoked, revocation_time, crl_updated}` | 10/h | `swarm:admin` | `admin_id` | `admin_critical` | `admin_scope` | `per_admin` |
| `swarm_auth_cert_list` | `{status}` | `{certificates: [{cert_id, subject, fingerprint, issued_at, expiry, status}]}` | 30/min | `swarm:admin` | `admin_id` | `read_replay_safe` | `admin_scope` | `per_admin` |
| `swarm_auth_device_register` | `{player_id, device_id, device_name, device_fingerprint, public_key, metadata}` | `{registered, device_id, registration_time, requires_approval}` | 30/min | `swarm:auth` | `player_id` | `idempotent_mutation` | `owner` | `per_player` |
| `swarm_auth_device_list` | `{player_id}` | `{devices: [{device_id, device_name, registered_at, last_seen, status}]}` | 30/min | `swarm:auth` | `player_id` | `read_replay_safe` | `owner` | `per_player` |
...
| `arena` | Arena (含 `swarm_get_leaderboard` — Arena 竞争排行) | Arena host / 赛事管理员 |
...
WASM 模块通过 host function import 调用引擎服务。以下为权威签名与限制。**共计 6 个函数**。
...
> **`host_get_random` domain separation**: `sequence` 参数提供 domain separation——不同调用上下文使用不同的 `sequence` 值确保独立随机流。引擎内部 PRNG 以 `(tick_seed, player_id, drone_id, sequence)` 为种子，保证同一 tick 内不同 drone/sequence 产生独立且可 replay 的随机序列。该函数为只读查询，不改变世界状态。
...
| **Starting resources** | `{Energy: 5000, Minerals: 2000}` | 新玩家初始资源包（R23 D1/A） |
...
| **Repair cap** | 3500 bp (35%) | Controller repair 免费比例（R23 D4/A） |
...
| 参数 | 值 | 说明 |
|------|-----|------|
| **Max concurrent sessions per player** | **5** | Per-player session cap |
| **Session idle timeout** | **30m** | |
| **Session absolute timeout** | **24h** | |
| **Refresh token lifetime** | **7d** | |
| **Access token lifetime** | **15m** | |
| **Cert validity max** | **365d** | |
| **Failed login lockout threshold** | **10 attempts** | |
| **Failed login lockout window** | **15m** | |
| **Failed login lockout duration** | **30m** | |
...
|---|-------|--------|--------------|------|
| 1 | `auth_login` | `tick, player_id, session_id, device_id, credential_type, ip_address, success, rejection_code` | `non_replayable` | Emitted when client attempts authentication via swarm_auth_login |
| 2 | `auth_logout` | `tick, player_id, session_id, initiator` | `non_replayable` | Emitted when session is terminated via logout or revocation |
| 3 | `auth_token_refresh` | `tick, player_id, session_id, success, rejection_code` | `non_replayable` | Emitted when client refreshes access token via swarm_auth_refresh |
| 4 | `auth_token_revoke` | `tick, actor_id, target_player_id, target_session_id, reason, affected_sessions` | `admin_critical` | Emitted when tokens are revoked via swarm_auth_revoke |
| 5 | `auth_cert_rotate` | `tick, old_cert_id, new_cert_id, new_fingerprint, reason` | `admin_critical` | Emitted when certificate is rotated via swarm_auth_cert_rotate |
| 6 | `auth_device_register` | `tick, player_id, device_id, device_name, device_fingerprint` | `idempotent_mutation` | Emitted when device is registered via swarm_auth_device_register |
...
| Payload | `{sub (PlayerId), sid (string), scope (string), did (string), iat (u64), exp (u64), jti (string)}` |
...
| Entropy | 256 bits |
| Encoding | base64url |
| Rotation | single-use; each refresh issues new refresh token and revokes old one |
| Family Tracking | Refresh tokens belong to token family (per session). Reuse triggers revocation of entire family to prevent replay attacks. |
...
## 11. Deploy — deploy_mutation 机制 (R16 B6)
...
**来源 IDL**: game_api. **权威合同**: `specs/core/05-persistence-contract.md` §2 Replay-Critical Subset + §2.3 Deploy 状态机。

### 概述

Persistence 层处理大型二进制对象的存储。Per D5/B + R22 B1：FDB 事务原子提交 **replay-critical subset**（10 项必填字段，见 `05-persistence-contract.md` §2.1）；object-store blob 上传为**异步**，不阻塞 FDB 提交路径。FDB 仅存储含 blob hash pointer 的小型 manifest record。这解耦了 I/O 吞吐与事务延迟。**Blob 缺失不影响 deterministic replay**——仅降级 rich audit（`terminal_state = audit_gap`）。
