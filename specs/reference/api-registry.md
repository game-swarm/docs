...
| 工具名 | 分类 | RFC 追踪 | 来源 IDL |
|--------|------|----------|----------|
| `swarm_list_market_orders` | Play | RFC-MARKET-001 | game_api |
...
| 2 | **StorageTax** | taxation | Every tick, per player | Tiered tax on global storage: 0 bp (<10K), 1 bp (10K–100K), 5 bp (100K–1M), 20 bp (1M+). Min 1 unit if rate > 0. |
| 3 | **UpkeepDeduction** | maintenance | Every tick, per player | Per-structure cost: 1 unit/tick. Controller upkeep: `level² × 10`. Drones have no passive upkeep. |
...
| 7 | **AlliedTransfer** | transfer | TransferToGlobal + TransferFromGlobal (allied only) | Tax-free transfer between allied players via global storage. No cooldown. Exact amount: sender deduction = receiver credit. |
...
| **StorageTax** | 1. Determine tier from stored_total. 2. `tax_deduction = (stored_total * rate_bp) / 10000` (u64, u32 → u64). Min 1 unit if rate_bp > 0 and stored_total > 0. | floor |
