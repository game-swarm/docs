...
> **权威工具清单见 [API Registry](api-registry.md) §3** — 工具数量与分组以 Registry 三口径为准（Game API `all_declared=57` / `active_only=53` / `rfc_gated=4`；Auth API `all_declared=12` / `active_only=12` / `rfc_gated=0`）。本文档提供使用模式和安全约束说明。
>
...
| Arena | `all_declared=4`, `active_only=1`, `rfc_gated=3` | `swarm_match_result` active；锦标赛编排为 RFC-gated |
...
| **Auth API** | **`all_declared=12`, `active_only=12`, `rfc_gated=0`** | `auth_api.idl.yaml` CSR lifecycle + device/recovery/federation 工具 |
...
Arena 当前 active 工具为 `swarm_match_result`；`swarm_tournament_create`、`swarm_tournament_precommit`、`swarm_tournament_status` 为 RFC。SDK/Resources 工具分别提供 SDK 拉取与资源定义读取。
