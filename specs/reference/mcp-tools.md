...
1. **环境感知**: 调用 `swarm_get_info` 获取当前 Tick 和世界配置，调用 `swarm_get_snapshot` 获取实体的结构化快照。
...
客户端通过 Gateway 的 request/response `POST /mcp` 调用工具；当前实现不提供 SSE subscription。JSON-RPC envelope 由 MCP 客户端与 Gateway 处理，本页不维护独立 wire 示例。每个工具的参数、结果字段、rate limit、scope、replay class 与 visibility filter 以 [API Registry §3](api-registry.md) 为准。
...
| Arena | `all_declared=4`, `runtime_active=4`, `implementation_feature_gated=0` | Tournament create/precommit/status and match-result tools are runtime-active |
| Resources | 2 | `resources/list` / `resources/read` |
| **Game API 小计** | **`all_declared=51`, `active_only=50`, `rfc_gated=1`** | `game_api.idl.yaml` 三口径工具统计 |
...
> 权威定义见 [API Registry](api-registry.md) §3.2 Onboarding (10) + Play (15)。
...
> 权威定义见 [API Registry](api-registry.md) §3.2 Auth (3 个 Game API 简化工具) 与 §3.3 Auth API 工具 (7 个完整 auth 工具)。

### 锦标赛 / SDK / Resources

> 权威定义见 [API Registry](api-registry.md) §3.2 Arena (`all_declared=4`, `runtime_active=4`, `implementation_feature_gated=0`)、SDK (1)、Resources (2)。

四个 Arena 工具均由 Engine 运行时分派。Engine 的 tournament-mode profile 还会按比赛准备流程提供一组经过筛选的查询、校验和部署工具；该 profile 不是 Arena category 的工具计数。SDK/Resources 工具分别提供 SDK 拉取与资源定义读取。
