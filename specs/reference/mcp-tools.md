...
> **权威工具清单见 [API Registry](api-registry.md) §3**。工具名称、数量和分组只从 IDL 生成；不要在本页维护独立计数。本文档提供使用模式和安全约束说明。
>
...
| Arena | `all_declared=4`, `active_only=4`, `rfc_gated=0` | Arena rows are active machine entries with extension status notes where applicable |
...
> 权威定义见 [API Registry](api-registry.md) §3.2 Arena (`all_declared=4`, `active_only=4`, `rfc_gated=0`)、SDK (1)、Resources (2)。

四个 Arena 工具均为 active machine entries。Engine 的 tournament-mode profile 还会按比赛准备流程提供一组经过筛选的查询、校验和部署工具；该 profile 不是 Arena category 的工具计数。SDK/Resources 工具分别提供 SDK 拉取与资源定义读取。
