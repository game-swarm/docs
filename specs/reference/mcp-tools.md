...
> **权威工具清单见 [API Registry](api-registry.md) §3** — 工具数量与分组以 Registry 为准（当前 Game API 57、Auth API 12）。本文档提供使用模式和安全约束说明。
>
...
| Arena | 1 active + 3 RFC | `swarm_match_result` active；锦标赛编排为 RFC |
| Resources | 2 | `resources/list` / `resources/read` |
| **Game API 小计** | **57** | `game_api.idl.yaml` active tools |
...
> 权威定义见 [API Registry](api-registry.md) §3.2 Arena (1 active + 3 RFC)、SDK (1)、Resources (2)。
