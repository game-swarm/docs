...
签名:  部署 WASM 时客户端用 CodeSigningCertificate 对 wasm_module_hash + metadata 签名 → 服务端验签
...
```json
{
  ...
  "compiled_artifact_hash": "blake3:789abc...",
...
| `compiled_artifact_hash` | `Blake3(compiled artifact bytes)`，服务端编译后写入；用于编译缓存与运行时 artifact 完整性，不得替代 `wasm_module_hash` 做代码签名 |
...
- MCP application certificate **不得**用于 WebSocket 连接；`agent-mcp`、`agent-ws`、`spectator-ws`、`cli-rest` transport 不可互换
- Deploy `DeployPayload.audience` 字段同上规则，必须为实际入口 transport（通常为 `agent-mcp` 或 `cli-rest`）
...
2. 客户端构建 DeployPayload：
   ...
   - compiled_artifact_hash 由服务端编译后计算，客户端不得自报
   - metadata_hash = Blake3(mod.toml)
   - signed_at = 客户端当前时间戳
...
4. RuleMod actions（确定性顺序：mod_id → action_index）
5. MCP_Query（只读，不产生副作用，顺序无关）
