...
> 详见 design/interface.md。本文档汇聚 specs/core/tick-protocol.md §4、specs/security/mcp-security.md §2、specs/security/visibility.md §3、specs/security/command-source.md §7.0 中关于 Gateway 的分散定义。
...
```
                    ┌──────────────┐
  Browser ──WS─────→│              │
  CLI     ──MCP/POST→│   Gateway    │──NATS──→ Engine
  Agent   ──MCP/POST→│Rust (axum), 无状态│←─HTTP──→ Engine (health)
                    └──────────────┘
```
...
|-----------|------|------|------|---------|
| **Browser Realtime** | WebSocket (`GET /ws`) | 8082 | 人类玩家 Web UI 实时状态推送 | Web session token 或 `Swarm-Certificate` + `Swarm-Cert-Id` + signed request |
| **Tool Calls** | MCP over HTTP (`POST /mcp`) | 8082 | Browser、AI agent 与 CLI 的 request/response 工具调用 | 按调用方 transport 使用 Web session 或 application certificate + signed request |
| **Health** | HTTP (`GET /healthz`) | 8082 | Gateway/NATS relay 健康检查 | 无业务数据访问 |
| **SDK Fetch** | HTTP (`GET /sdk/:lang`) | 8082 | SDK starter package | 与 Gateway 请求认证一致 |

> **注意**: WebSocket 路径 (`/ws`) 负责实时状态订阅；所有工具调用（含 MCP）通过 HTTP POST (`/mcp`)。Gateway 负责将 HTTP `/mcp` 请求代理到 Engine，并将 Engine NATS `swarm.realtime.v1` 消息桥接到浏览器 WS。当前无 SSE 实现。


**判定规则**（specs/security/command-source.md §7.0）：
...
Delta 仅包含对 subscriber 可见的实体——经 `is_visible_to(subscriber, tick)` 过滤（specs/security/visibility.md §3.3）。
...
MCP transport 遵循标准 MCP 协议（JSON-RPC over HTTP POST）。Gateway 作为 MCP 反向代理，将请求转发至 Engine：

```
Agent → Gateway (POST /mcp) → Engine (POST /mcp)
...
Sandbox deploy/tick subjects 除 TLS/ACL 外还必须使用 `SWARM_NATS_AUTH_SECRET` HMAC 信封。Gateway realtime 只转发包含非空目标 player/session 的消息，不允许无目标广播。Gateway 应用层认证 nonce store 使用 `SWARM_GATEWAY_NONCE_PATH` 持久化 replay 状态；Engine 对 Gateway proxy request 的 nonce replay 状态使用 `SWARM_PROXY_NONCE_PATH` 持久化。生产部署必须将这些路径放在 `/tmp` 以外的可写持久卷上，Engine nonce store 父目录必须为 engine 用户所有、私有权限且不能是 symlink；读取、解析或原子写入失败时认证 fail closed。规则模组是进程内 Bevy Plugin；安全深度还包括 Gateway 应用层认证、mod 源码审查与可审计 hook/schedule graph。
...
| Rate limiting | 按 player + transport 独立限流（见 mcp-security.md） |
