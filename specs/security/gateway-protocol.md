...
| **Replay Viewer** | HTTP/1.1 | 8082 | (Planned) 回放查看器 | Application certificate 或匿名 (public replay) |

> **注意**: HTTP `/replay` 路由当前未在 Gateway 中注册，Replay Viewer 仍是计划功能。历史回放已可通过实现的 MCP `swarm_get_replay` 工具读取；WebSocket 路径负责实时状态订阅。
