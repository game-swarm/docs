...
Client → Gateway:  wss://<host>/ws
  Upgrade request includes `Sec-WebSocket-Protocol: swarm-cert.<cert_id>` header (certificate-based)
  or `Sec-WebSocket-Protocol: swarm-session.<token>` header (Web session token for browser UX)
...
| `/metrics` | GET | Public edge 基础指标 | public-safe |
| `/internal/metrics` | GET | Gateway ↔ Engine 内部指标 | internal network ACL |
| `/admin/metrics` | GET | 高基数与安全敏感指标 | admin certificate |
...
不要求消息级 MAC 或签名信封。规则模组是进程内 Bevy Plugin；被攻破的 Engine 可以生成任意合法消息，消息签名没有额外安全收益。安全深度由 NATS TLS/ACL、Gateway 应用层认证、mod 源码审查与可审计 hook/schedule graph 共同提供。
...
| 措施 | 说明 |
|------|------|
| Certificate audience 绑定 | 证书的 `audience` 必须匹配 `transport:server_id:world_id:player_id` 格式 |
