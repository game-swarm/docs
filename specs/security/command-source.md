...
### 7.0 Transport Binding 与 Browser/Agent 判定

应用层证书请求签名绑定 `cert_id`、`player_id` 与实际 transport，防止跨 transport 重放。JWT `aud` 仅用于 Web session 兼容路径：
...
| REST (CLI) | signed `cert_id` + `player_id` + `X-Swarm-Transport: rest` | HTTP header `X-Swarm-Transport: rest` + application certificate |
| Replay (Viewer) | replay transport or anonymous public replay | HTTP header `X-Swarm-Transport: replay` |
...
2. 客户端构建 DeployPayload：
   ...
   - metadata_hash = Blake3(mod.toml)
   - transport_binding = 实际入口 transport + cert_id + player_id
   - signed_at = 客户端当前时间戳
...
Deploy 不进入 RawCommand queue。RawCommand queue 只包含 gameplay command（WASM/Admin/TestHarness/Tutorial 等允许写世界的来源）与只读 query 的排序记录；MCP_Deploy 是控制面 mutation，按 §7.3 的 `version_counter` 与 redb `activation_tick >= current_tick + 1` 生效。

同 tick 内多个 gameplay RawCommand 按以下顺序处理：

1. Admin 命令（优先执行，允许影响全局状态）
2. WASM tick() 输出命令（每个 drone 独立处理）
3. MCP_Query（只读，不产生副作用，顺序无关）
