...
以下 CommandAction 在 Phase 0 冻结后新增，锚定 DESIGN.md §5, §8.2。

### A.1 RangedAttack
远程攻击。drone 需 RangedAttack body part。伤害 = parts × 25，范围 3。
```json
{ "action": "RangedAttack", "object_id": "d1", "target_id": "e5", "range": 3 }
```

### A.2 ClaimController
占领 Controller。drone 需 Claim body part。范围 1。
```json
{ "action": "ClaimController", "object_id": "d1", "target_id": "c1" }
```

### A.3 Recycle
回收 drone。退还 50% 资源。Tutorial 世界前 500 tick 退还 100%。
```json
{ "action": "Recycle", "object_id": "d1" }
```

### A.4 特殊攻击 (6 种)
锚定 DESIGN.md §8.2 特殊攻击方式表。每种有独立冷却 + 资源消耗 + 抗性:
- Disrupt (Attack): 打断目标动作, 50 tick CD, 100E, Sonic 抗性
- Fortify (Tough): 护盾×0.5 + 净化, 300 tick CD, 400E
- Hack (Claim): 夺取 drone 转 Neutral, 200 tick CD, 1000E, Psionic 抗性
- Drain (Carry+Work): 窃取资源, 50 tick CD, 200E/tick, EMP 抗性
- Overload (RangedAttack): 消耗配额 -500k, 200 tick CD, 300E, EMP 抗性
- Debilitate (Work): 易伤×2, 150 tick CD, 200E, Corrosive 抗性
- Leech: 吸血 (custom_action), Corrosive, 15 dmg, 300E
- Fabricate: 转化建筑 (custom_action), 500 tick CD, 2000E+500M
