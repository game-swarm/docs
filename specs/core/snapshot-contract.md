...
`omitted_categories` 的玩家可见值为枚举分桶：`0` / `few` / `some` / `many` / `extreme`。即使某一类未省略，键也必须存在（值为 `0`），以保证 schema 稳定性。精确省略计数只允许进入 admin/debug 内部 trace，不进入玩家 WASM snapshot、MCP 普通查询或公开 replay。
...
### 3.3 extension功能（Extension）

以下经济特性明确划入extension（Extension），当前不实现：

| 特性 | extension 标记 | 说明 |
|------|:-------:|------|
| Contract Settlement | `extension-CONTRACT` | 智能合约结算，条件触发式转移 |
| Merchant NPC | `extension-MERCHANT` | NPC 商人，固定汇率买卖 |
| Drone P2P Offer | `extension-P2P` | 无人机间点对点报价交易 |
| Auction House | `extension-AUCTION` | 拍卖行 |
| Escrow Service | `extension-ESCROW` | 第三方托管服务 |
| Resource Lending | `extension-LEND` | 资源借贷（含利息） |
