...
> `design/economy-balance-sheet.md`、`design/gameplay.md` §2.2 (资源与经济)、`design/engine.md` 中所有经济计算必须引用本节参数，不得独立定义公式。
...
5. ControllerPassiveIncome (Controller 被动收入 → world faucet 预算)
6. WreckageSalvage     (残骸回收 → world faucet 预算，按 tick 衰减)
7. LocalTransfer       (drone 间转移)
8. GlobalDeposit       (本地→全局)
9. GlobalWithdraw      (全局→本地)
10. AlliedTransfer     (联盟转移 → 按预算裁决)
11. BuildCost          (建造消耗)
12. SpawnCost          (生成消耗)
13. RecycleRefund      (回收退还)
```
...
|----------|-------------|-----------------|--------------------------|
| Contract | `player:{owner} → reserve:Contract:{id}:input` | Same-resource settlement returns `output_amount` to owner and sends remainder to `sink:contract_remainder`; cross-resource settlement sends input to `sink:contract_input` and output from `system:contract_output` to owner | Owner or configured counterparty may settle; duplicate settle is idempotent after auth |
| Merchant trade | No player reserve at quote creation | `player:{buyer} → merchant:{quote_id}` for pay resource, then `merchant:{quote_id} → player:{buyer}` for receive resource | Quote must be open and unexpired; `min_receive` protects buyer |
| P2P offer | `player:{maker} → reserve:P2POffer:{id}:give` | Taker pays maker directly for want resource; reserve releases give resource to taker | Maker/taker constraints enforced before receipt idempotency |
| Auction | Seller reserves lot; each bidder reserves bid | Prior high bid is refunded `reserve:Auction:{id}:bid → player:{prior_bidder}` before new bid reserve; settle releases lot to winner and bid to seller | Bids after `ends_at` reject; settle/cancel require authorized actor |
| Escrow | `player:{payer} → reserve:Escrow:{id}:escrow` | Release sends reserve to payee; refund returns reserve to payer | Payer or arbiter may release; refund policy requires the authorized party and open status |
| Lending | `player:{lender} → reserve:Lending:{id}:principal` | Accept releases principal to borrower; repay transfers `repay_amount` borrower → lender; default records status only and does not mint or move resources | Default is allowed after `due_at` and is idempotent status finalization |

---

## 8. 与现有文档的关系

- `design/gameplay.md` §2.2 (资源与经济): 本文档为权威实现合同。
