...
└─────────────────┼────────────────────────┘
                  ...
                  ▼
┌──────────────────────────────────────────┐
│  Sandbox Worker shared pool（worker 无状态） │
│  ┌────────────────────────────────────┐  │
│  │  OS 隔离:                           │  │
│  │  seccomp(bpf) — 白名单系统调用       │  │
...
**生命周期**: Sandbox 采用 **long-lived worker pool** 模型。Deploy 只接收 Engine 转发的原始 WASM，并在 sandbox 进程内预编译和缓存；不接受任何调用方提供的 native artifact。缓存键使用 `Blake3(module_hash || wasmtime_build_commit || wasmparser_version || validation_policy_version || target_arch || security_epoch)`。Engine 验证客户端签名的 `wasm_hash + metadata_hash + identity/slot/version/transport`；Sandbox 验证 Engine NATS HMAC envelope，编译后返回 `compiled_artifact_hash` attestation。缓存仅跳过重复编译，不跳过部署验证。每 tick：worker 从池中取出 → 执行 **Store reset checklist**（以下所有步骤按序执行，任一失败 → worker 替换并审计）:
...
→ 执行单一玩家的 `tick()` → 返回 ABI v2 `TickResult` bytes → 返回池中。
...
- Wasmtime 版本的安全支持窗口：跟踪 Bytecode Alliance 的 LTS/non-LTS 发布策略，锁定版本需在官方安全支持窗口内
...
```rust
fn validate_module(module_bytes: &[u8]) -> Result<(), Rejection> {
    // 1. 体积检查
    if module_bytes.len() > 5 * 1024 * 1024 {
...
## 3. Command Intent Model — 延迟指令模型

WASM 模块采用 **registered model**：`tick()` 接收 ABI v2 `TickInput` bytes，**返回 ABI v2 `TickResult` bytes**。两者都使用 IDL-generated Swarm codec；JSON/v1 只允许作为调试显示格式，不能作为 tick 或 host wire format。引擎解码 `TickResult.commands` 并在校验后执行指令。WASM 中**不得直接调用 mutating host function**——所有状态变更必须通过 `TickResult.commands` 返回，由引擎统一应用。
...
// 主入口
tick(input_ptr: i32, input_len: i32, output_ptr: i32, output_len: i32) -> i32;
//   input_ptr/len: 引擎写入的 TickInput Swarm codec bytes
//   output_ptr/len: 引擎提供的 TickResult 输出 buffer
//   返回值: 写入字节数；负数 = ABI 错误码
//
//   调用协议:
//   1. 引擎 alloc input_len → 写入 TickInput bytes
//   2. 引擎 alloc output_len → 作为 output_ptr
//   3. 调用 tick(input_ptr, input_len, output_ptr, output_len)
//   4. 若返回值 >= 0，读取 output_ptr[..bytes_written]
//   5. 校验 bytes_written <= 256KB → 解码 TickResult
//   6. 调用 free(output_ptr, output_len) 释放输出 buffer
//   7. 调用 free(input_ptr, input_len) 释放输入 buffer
```

**安全约束**:
- 所有 pointer/len 做 bounds check、alignment check、integer overflow check
- TickResult bytes 超过 256KB 或 Swarm codec 解码失败 → 拒绝该玩家当 tick 所有输出
- tick() 返回负数 → ABI failure；非负值是实际 bytes_written
...
fn host_get_fuel_remaining() -> u64;  // 只读；返回当前 tick 剩余 Wasmtime fuel
```
...
- ❌ `host_move` / `host_harvest` / `host_transfer` / `host_build`
- ❌ `host_attack` / `host_heal`

上述 mutation 必须作为 ABI v2 Swarm codec `TickResult.commands: CommandIntent[]` 返回；JSON 仅为文档展示，不是 wire。
...
`write` 仅允许写 sandbox-worker 预绑定的内部 pipe/eventfd；WASI stdout/stderr 仍禁用，玩家 WASM 不能通过 `fd_write` 输出任意日志或观察宿主文件描述符。`clock_gettime` 保持禁止；Wasmtime epoch interruption 由宿主线程驱动，不要求 sandbox 内执行时钟 syscall。
...
| clone flags | 默认 | 说明 |
|-------------|------|------|
| `CLONE_VM, CLONE_FS, CLONE_FILES, CLONE_SIGHAND, CLONE_THREAD, CLONE_SYSVSEM, CLONE_SETTLS, CLONE_PARENT_SETTID, CLONE_CHILD_CLEARTID` | ✅ 允许 | 仅限 runtime 线程组合；加必要 signal flags |
...
sandbox worker 进程拥有**独立 netns**，无网络接口、无路由表、无 iptables 规则。guest WASM 没有任何网络能力；所有跨进程通信都由 sandbox worker 进程的控制面完成：worker 在沙箱外层接收 NATS request-reply 任务、把 `TickInput` bytes 写入 guest 线性内存、读取 `TickResult` bytes 后通过 NATS reply 返回。不得向 guest WASM 传入 Unix socket、network fd 或其他可用于直接通信的句柄。
...
| host_get_fuel_remaining 调用 | 无单独次数上限 | 只读，按基础 fuel cost 计入总 host call budget |
| TickResult Swarm codec bytes | 256 KB | 返回值大小检查 |
...
超限 → 返回错误码 + 审计日志。fork allocation/copy/isolation 失败返回 `SimulationUnavailable`，不回退共享 Store/authoritative World，不返回 partial output。长模拟必须使用离线或异步隔离 worker。
...
| 字段 | 限制 | 超限处理 |
|------|------|---------|
| RawCommand body | 1 KB（hash + truncated preview） | 完整 body 丢弃，保留 `command_id` + 前 200 字符 |
...
pre-warm 机制降低部署延迟和首 tick 冷启动 P99，需要 pre-warm 机制降低 P99 延迟。
...
**预编译触发**：Sandbox 在 deploy manifest commit 前完成编译、shared Artifact Store primary+backup 写入和 DeployAck attestation。不存在 commit 后后台编译或 audit blob upload gate。
...
| 模块缓存 | `Blake3(length_delimited(compiled_artifact_hash, wasmtime_build_commit, wasmparser_version, validation_policy_version, target_arch, security_epoch))` | Sandbox 派生并 attested；epoch/policy 变化全量失效 |
| 并发编译 | 最多 5 个 | 防止编译阶段 DoS |
| module validation | 10ms | wasmparser 解析超时 |

## 8. Query Host Function 单次调用成本表

以下仅列出 §3.2 允许的**查询类 host function**。Mutating 操作的 cost 定义在 specs/gameplay/api-idl 各 command 的 `cost` 字段中。

| 函数 | fuel 成本 | 响应大小上限 | 说明 |
|------|----------|------------|------|
| `host_get_terrain` | 500 | 8 KB | |
| `host_get_objects_in_range` | 2,000 + 100/entity | 64 KB | |
| `host_path_find` | 500 × explored_nodes + 200 × expanded_edges | 8 KB | 与 Registry 一致；cache hit/miss 不加独立 penalty。上限 10 次 + 100,000 explored_nodes/player/tick。 |
| `host_get_world_config` | 1,000 | 16 KB | |
| `host_get_world_rules` | 1,000 | 16 KB | |
| `host_get_random` | 200 + 10/32 bytes | 256 bytes | 确定性随机；派生规则见 §3.2 `derive_rng` 规范 + `host-functions.md`；per_tick_limit=10 |
| `host_get_fuel_remaining` | 20 | 8 bytes | 返回当前 Store 剩余 fuel；无单独次数上限 |
...
生产环境 **禁止** `sandbox.relaxed = true`。引擎启动时检查进程环境；若为 true 且 `SWARM_ENGINE_MODE != "development"` → 拒绝启动。`development` 不是 world.mode wire value。
