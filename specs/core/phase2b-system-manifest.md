...
> 详见 design/engine.md
>
> **修复**。本文档是 Swarm 引擎**全部 tick 系统执行顺序的唯一权威定义**——覆盖 System Pass 2a inline 命令处理器与 System Pass 2b 被动系统。所有其他文档（engine.md、tick-protocol.md、command-validation.md）引用此处，不得重新声明可冲突的系统列表或顺序。
>
> **修复**：Status Effects 拆分为「并行 buffer 生产」(S16-S21) + 「串行唯一 committer」(S22)。S22 移出 Parallel Set B。combat 与特殊攻击不再由 S01 直接处理：Attack/RangedAttack/Heal 经 Action dispatch 进入 combat intent buffer，special actions 经 ActionRegistry handler 进入 status intent buffer。
...
**共计 31 个 system**（ 新增 S22a leech_buffer + S22b fabricate_buffer，共 31）。**A01 action_dispatch 是 System Pass 2a per-command handler，不计入 manifest system 表**。所有特殊攻击状态推进由 `status_advance_system` (S22) **唯一**串行处理——S16-S22b 只写 typed buffer，不直接修改 StatusState。
...
> **System Pass 2a 内联执行**：这些系统在命令循环中逐条 inline 调用。每条命令校验基于**当前 Bevy World 状态**，合法则立即应用。所有效果在后续同一 tick 的系统可见。
...
- **Note**: System Pass 2a 校验 + 扣费 + 写入 `PendingEntityCreation`。实际 drone 创建由 System Pass 2b S08 spawn_system flush。新 drone 的 `StableEntityId` 可写入 trace，但实体数据不进入本 tick 可见/可交互索引；下一 tick 才参与快照、命令校验和系统迭代。

---

### System Pass 2b: System Pass (S07–S31)
...
- **ID**: `spawn_grace`
- **fix**: 移至 combat 之前，确保出生 tick 无敌保护生效。
...
S11-S13 不产生特殊攻击 intent。特殊 action intent 由 A01 ActionRegistry handler 写入 status intent buffer。
...
S16-S22b **只读**现有 StatusState，**只写** typed effect buffer。不直接修改任何 StatusState component。所有 buffer 由 S22 统一消费。
...
| debilitate_buffer | `debuff_buf` | DebilitateState, Entity (diligence) | `DebilitateBuffer { target, factor }` |
| disrupt_buffer | `disrupt_buf` | DisruptState, Entity (action, body_parts) | `DisruptBuffer { target, interrupted }` — **要求 body part match**（ |
| fortify_buffer | `fort_buf` | FortifyState, Entity (armor) | `FortifyBuffer { target, armor_delta }` |
| leech_buffer | `leech_buf` | LeechState, ResourceAmount, Entity (age) | `LeechBuffer { target, resource_drain, age_transfer }` — **新增 ()** |
| fabricate_buffer | `fab_buf` | FabricateState, Drone (body) | `FabricateBuffer { target, body_part_mod }` — **新增 ()** |

**Parallel safety ()**: 各 system 写入互不重叠的 typed buffer（`HackBuffer` ≠ `DrainBuffer` ≠ …）。所有 buffer 与 StatusState component 分离——S16-S22b 不写任何 StatusState，S22 作为唯一 StatusState writer 串行执行。无并行写入冲突。
...
- **Writes**: **All StatusState components** (`HackState`, `DrainState`, `OverloadState`, `DebilitateState`, `DisruptState`, `FortifyState`, `LeechState`, `FabricateState`), Entity (armor/diligence/interrupted via effect application), ResourceAmount (drain), FuelBudget (overload)
...
for each intent in pending_intents (canonically sorted):
    match intent.kind:
        ...
        Debilitate → Entity.diligence *= debilitate_factor; duration = debilitate_duration
...
// Pass 2: Apply buffer effects from S16-S22b (ongoing status tick effects)
for each entity with active StatusState:
    ...
    if DebilitateBuffer[entity] → apply diligence modifier
...
// Pass 3: Decrement durations + expire
...
| `LeechState` | `status_adv` (S22) | S22 Pass 1 — apply S14 intents **(新增)** |
| `FabricateState` | `status_adv` (S22) | S22 Pass 1 — apply S14 intents **(新增)** |
...
| `LeechBuffer` | `leech_buf` (S22a) | S22a S22 Pass 2 input |
...
| 2.0.0 |  | 修复：31 systems。S14 special_attack_reducer、S06 spawn_validator。 |
| 3.0.0 |  | **修复**：Status Effects 拆分——S16-S22b 并行 buffer 生产 + S22 串行唯一 StatusState committer。新增 S22a leech_buffer + S22b fabricate_buffer（31 systems）。S01 写入 PendingSpecialAttackIntent。8 种特殊攻击全部核心目标。S14 从 S01 读取 intents。Unique Writer Contract 完备化。 |
