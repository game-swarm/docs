...
```
                 ...
                 │  │ 对每个活跃玩家:           │     │
                 │  │ 1. 构建可见性快照          │     │
                  │  │ 2. NATS 调用 WASM worker  │     │
                 │  │ 3. 超时 → 空指令列表      │     │
                 │  └─────────────────────────┘     │
                  │  结果: Map<PlayerId, TickResult>         │
                 └──────────┬───────────────────────┘
                            │
                            ▼
                 ┌──────────────────────────────────┐
                 ...
                 │  │ intent 不直接改 HP         │     │
                  │  │ Spawn → 稳定校验+扣费+写入 │     │
                  │  │ ProvisionalSpawnRequest    │     │
                 │  └─────────────────────────┘     │
                 │  ┌─────────────────────────┐     │
                 │  │ System Pass 2b: ECS Systems     │     │
                 │  │ death_mark → spawn →     │     │
...
唯一执行器：**WasmSandboxExecutor**。所有玩家的 drone 都通过 WASM 沙箱执行——无论是人类编写还是 AI agent 编写。没有 McpPlayerExecutor，也没有本地/Unix 专用执行 profile；Engine 始终通过 NATS request-reply + shared queue group 调度无状态 sandbox worker pool。
...
fn stitch_player_snapshot(world_snapshot, player_id) -> Snapshot:
    ...
    return Snapshot {
        ...
        resources: payload.resources,    // [Resource]，玩家自身/可见资源
        events: payload.events,
        truncated: payload.truncated,
        over_budget: payload.over_budget?, // optional; true only on deterministic minimal path
        messages: payload.messages,
        omitted_categories: payload.omitted_categories,
        omitted_messages: payload.omitted_categories.messages,
    };
```

- 快照按房间分片序列化一次，再为玩家 stitch 可见 shard——不是对 `all_entities` 做 per-player 全量过滤。
- 超限时使用 [Snapshot Contract §4](../core/snapshot-contract.md) 下沉的 truncation 算法；该合同与本文件共同派生自 design，tick-protocol 不重复定义独立算法。
  - 截断算法（messages-first + 距离桶 + cross-category stable_item_key + farthest-first + critical 不可截断）全部由 snapshot-contract 定义。
...
| 滥用模式 | 检测方法 | 响应 |
|---------|---------|------|
| 实体膨胀攻击 | 玩家可见实体数连续 5 tick 超过 `MAX_VISIBLE_ENTITIES`（500） | 记录 `visibility_pressure_source`，对产生压力的源实体或玩家限额或截断；不降低受害者 COMBAT 优先级 |
| 出口视野扩展 | 单 tick 内可见房间数 > 9（默认 ≤9） | 截断到 9 房间，记录审计日志 |
| 截断频率异常 | 连续 3 tick `truncated=true` | 告警；对压力来源执行限额或分桶降级 |
...
关键不变量：
- 步骤 [4] WASM tick() 和步骤 [5] MCP query 基于**同一份**快照——`snapshot.tick == current_tick`
- 快照在 COLLECT 阶段构建一次，COLLECT 期间不变（redb commit 失败回滚时使用同一快照恢复）
- MCP query 不能观察到 EXECUTE 阶段的中间状态——只能看到 COLLECT 开始时的世界快照

### 2.4 WASM 模块部署

AI 玩家通过 MCP `swarm_deploy` 上传 WASM 模块。Deploy 是控制面 mutation，不进入 gameplay RawCommand queue，不参与 System Pass 2a 指令排序。服务端在 redb deploy manifest 中记录 `activation_tick >= current_tick + 1`，引擎只在 COLLECT 开始前切换到已到达 activation_tick 的模块：
```
...
```rust
// 每 tick 洗牌一次，使用 canonical domain-separated seed 派生确定性随机序列
// seed = Blake3("shuffle" || world_seed || tick_number.to_le_bytes())
// shuffle = Blake3 XOF: for i in 0..N:  position[i] = XOF.read_u64() % (N - i)
let seed = blake3::hash(&[b"shuffle", &world_seed, &tick_number.to_le_bytes()]);
let player_order: Vec<PlayerId> = seeded_shuffle(&active_players, &seed);

// 按洗牌后的玩家顺序 + 玩家内部指令序号 + canonical command id 排序
for (order_index, player_id) in player_order.iter().enumerate() {
    let mut player_commands = tick_results
        .remove(player_id)
        .unwrap_or_default()
        .commands;
    player_commands.sort_by_key(|c| c.sequence);
    for intent in player_commands {
        let raw = RawCommand::inject(*player_id, tick_number, intent);
        let command_id = blake3::hash(&canonical_swarm_codec(&raw));
        gameplay_raw_queue.push((order_index, *player_id, raw.sequence, command_id, raw));
    }
}

gameplay_raw_queue.sort_by_key(|item| (item.0, item.1, item.2, item.3));
```

Canonical gameplay order is exactly `(player_order, player_id, sequence, command_id)`. `command_id` is `Blake3(canonical RawCommand)` after server injection. Admin/emergency/audit operations are a separate control plane and never enter this gameplay RawCommand queue.
...
**种子轮换**：`world_seed` 每 10000 tick 固定轮换，防止长期观察推断种子空间。该周期是 replay protocol 常量，不是 world.toml 可配置项；变更需要 protocol version bump。

轮换算法：`new_seed = Blake3(old_seed || current_tick)`。替换前种子对应的回放数据仍可验证——TickCommitRecord 中记录每 tick 使用的 seed epoch，回放时按 epoch 选择对应种子。

**种子生命周期与泄露防护（— 混合方案）**：

> **根本约束**：在确定性系统中，后续状态（包括种子）可从当前状态推导——真正的密码学前向保密不可能。泄露了当前种子 + 状态的攻击者能模拟全部后续 tick。种子归档在快照/keyframe 中已足够用于确定性 replay。
...
Operator Seed-Bump MCP 工具：
  swarm_world_seed_bump { world_id, reason }
    ...
    → seed_epoch_id += 1
    → 替换前 seed 标记为 compromised（审计日志记录 bump reason）
    → 后续 tick 使用新 seed 派生
...
```
Source E1: resources["Energy"] = 5

排序后指令队列:
  1. Player B: harvest(E1) → 拿走 5，E1 剩余 0
2. Player A: harvest(E1) → 校验时发现 E1.resources["Energy"] = 0
...
| 治疗同一目标 | 按顺序加血，满血后 `CooldownActive` + `debug_detail=AlreadyFullHealth` |
...
### 3.3 指令执行模型（Inline + System Pass 2a TOCTOU 合同）

命令循环采用 **Inline 模型**：逐条校验 + 逐条应用，校验基于**当前** Bevy World 状态（非快照）。

**System Pass 2a TOCTOU 保护合同**：

以下规则防止 inline 执行中的时间窗口攻击——所有规则在 `validate_and_apply()` 单一路径中强制执行：

1. **Spawn pending 不可见**：System Pass 2a 中 S06 Spawn 命令只校验 stable facts、预留/扣费并写入 `ProvisionalSpawnRequest`。S08 是该 provisional queue 的唯一 consumer：在 S07 释放 RoomCap 后重新检查 volatile admission，消费 RoomCap，finalize cooldown，接受或退回 debit，并把 accepted spawn 追加到 `PendingEntityCreation`。S08 不创建 ECS 实体；tick-end creation flush（不属于 31 combined entries）是 `PendingEntityCreation` 的唯一 consumer/materializer，按 `StableEntityId` 排序 materialize accepted Build/Spawn/Fabricate。新实体不加入本 tick 可见/可交互世界索引；同 tick 后续命令无法看到、操作、或依赖尚未创建的 drone。新实体从下一 tick 开始参与快照、命令校验和系统迭代；`SpawningGrace { remaining: 1 }` 在首次可交互 tick 生效。
...
4. **fuel/wall-clock 耗尽**：WASM 执行中 fuel 耗尽或 wall-clock timeout → 完整输出丢弃（不读取部分输出），不计 refund。消息消耗 fuel：被接受的每条 message 成本为 `500 + 10 * byte_len`，按 `TickResult.messages` 原始顺序计费，直到 fuel 耗尽；耗尽后的 message 按 per-message invalid/drop 处理，不丢弃已接受的 commands 或 messages。
5. **指令队列不跨 tick**：超时玩家的指令输出仅丢弃当前 tick，不携带到下个 tick（防止 sequence 冲突与状态污染）。

非法指令 → 拒绝，记录 RejectionReason，写入 TickCommitRecord。

### 3.3.1 TickResult Messages

`TickResult.messages` 进入玩家消息队列，最早下一 tick 对 recipient 可见，不直接改变本 tick 世界状态。单条 message 无效（schema 错误、recipient 不可达、超过 fuel 或 size limit）时只丢弃该 message；其他 valid messages 与 commands 继续处理。

同一 tick 内 accepted messages 的 canonical order 固定为 `(sender_id, recipient_id, original_index)`。`original_index` 是该 sender 的 `TickResult.messages` 原始下标。消息队列 snapshot priority 最低：超预算时 message inbox 最先截断，`omitted_messages` 只记录 `0/few/some/many/extreme` bucket；精确数量仅进 admin trace。省略 message 不影响 gameplay command replay。

### 3.4 ECS 系统执行顺序

> 调度由 `design/engine.md` 定义，并在 [Complete Tick Execution Manifest](phase2b-system-manifest.md) 下沉为 6 个 Pass 2a inline handlers + 25 个 registered Pass 2b systems = 31 combined 的执行表。System Pass 2a inline 处理器在命令循环中逐条应用；System Pass 2b 按派生 manifest 执行。

**关键时序合同**：
- `death_marker` 在 `spawn` 之前：RoomCap 槽位同 tick 释放。
- `spawn_system` consumes `ProvisionalSpawnRequest` and appends accepted spawn creations to `PendingEntityCreation`；tick-end creation flush uniquely materializes accepted Build/Spawn/Fabricate, and new entities are earliest interactable next tick.
...
**Component R/W 矩阵**：见 [Complete Tick Execution Manifest §4](phase2b-system-manifest.md) — 覆盖 6 个 Pass 2a inline handlers 与 25 个 registered Pass 2b systems（31 combined）的读写关系、并行安全证明及 RoomCap 中间态保护。
...
struct GlobalTickCommit {
    ...
    manifest_hash: Blake3,                              // existing replay composite manifest digest
}
```
...
| Replay identity | collect_id、attempt_id、commit_id、api_version、engine_abi_version、system_manifest_hash、world_action_manifest_hash、mods_lock_hash、seed_epoch | 识别一次 collect/commit 尝试与固定 schedule/action registry 版本；用于审计和 hash-chain |
| `TickInputEnvelope` | module_hash、wasmtime_version、effective_tick、fuel_schedule_version、host_cost_table_version、system_manifest_hash、world_action_manifest_hash、world_config_hash、mods_lock_hash、wasm_status、deploy_events、rollback_events、admin_events、terminal_state | 记录 COLLECT 输入与运行环境；对象存储 rich trace 缺失时仍可由 redb core replay |

Deterministic replay 的权威输入是 redb 从 genesis 开始永久保存的 replay-critical log：`TickCommitRecord` + `TickInputEnvelope` + config/mod/deploy/admin transition records。keyframe/delta chain 只允许作为 verified acceleration；缺失、损坏或 hash 不匹配时必须从 genesis log 回放。Rich debug replay 可读取 RichTraceBlob 补充 per-system metrics、debug detail 和可视化 annotation；RichTraceBlob 缺失只产生 `terminal_state = audit_gap`，不产生 `unreplayable`。
...
**与替换前模型的区别**：
- ❌ 替换前：「单房间写入失败 → tick 放弃，全局快照恢复」（暗示 per-room 写入已 durable → 需要全局回滚）
- ✅ 新：「单房间 staging 写入失败 → tick 放弃，staging 行由 GC 清理」（staging 行从未 durable——无需回滚，仅需 forward abandon + GC）

#### Bevy World 快照范围清单

快照在 System Pass 2a 开始前完成，捕获完整的 World 状态。以下为必须捕获的 Resource 类型和所有 ECS Component 类型：
...
**快照生命周期**：
```
System Pass 2a 开始前: snapshot = world.snapshot()  // 深拷贝 Bevy World
System Pass 2a-2b:      在 world 上原地修改
redb commit 成功:  丢弃 snapshot
redb commit 失败:  world.restore(snapshot)      // 恢复所有 Component + Resource
```
`world.restore(snapshot)` 将 Bevy World 完全回滚至 System Pass 2a 前的状态，包括所有实体的 Component 数据、所有 Resource 数据。
...
```rust
#[test]
fn redb_commit_failure_restores_snapshot_consistency() {
    // 1. 构建初始 World 状态
    ...
    // 3. 执行 N 个 tick
    for tick in 0..1000 {
        let snapshot = world.snapshot();  // System Pass 2a 前快照
...
| **System Pass 2a panic/OOM** | inline apply 中 panic 或内存耗尽 | Bevy snapshot 恢复，tick 放弃 | 已消耗 fuel 不退，已执行玩家空 tick | 重试 3 次（复用 COLLECT 缓存）；失败降级 |
...
| **WASM output invalid** | tick 输出不符合 ABI v2 TickResult Swarm codec schema | 该玩家所有指令丢弃 | 空 tick，不退 fuel | 下 tick 正常（需玩家修复代码） |
...
- 暂停 deploy control-plane requests（禁止代码更新，防部署丢失）
- 保持已有玩家 WASM 执行
- 告警升级 → 需管理员介入
...
每个 tick 写入 redb（不可变、永久保留）：
```
/tick/{N}/commit_record         → TickCommitRecord replay-critical core
/tick/{N}/input_envelope        → TickInputEnvelope（module/config/fuel/host/deploy/admin 输入环境）
/tick/{N}/commands              → 全部玩家排序后的 canonical RawCommand（或 commit_record 内嵌等价字段）
/tick/{N}/rejections            → 被拒绝的指令及原因
/tick/{N}/transitions/config    → world config / ruleset transition（若本 tick 发生）
/tick/{N}/transitions/mods      → mods.lock / action manifest transition（若本 tick 发生）
/tick/{N}/transitions/deploy    → deploy activation/rollback decision（若本 tick 发生）
/tick/{N}/transitions/admin     → admin control-plane decision（若本 tick 发生）
/tick/{N}/metrics               → TickMetrics（非 replay 权威输入）
```

每 tick 只记录 `state_checksum` 与 manifest/hash 链，不记录 per-tick full world state。`/tick/{N}/metrics` 是 redb replay/audit 路径；Engine 另行提供 Prometheus HTTP `/metrics`，只暴露进程/readiness/aggregate 指标，不能泄露 world_seed、private state 或 replay payload。
...
```rust
fn replay_to(target_tick) -> WorldState {
    let mut state = if let Some(keyframe) = load_verified_keyframe_at_or_before(target_tick) {
        keyframe.state
    } else {
        genesis_state()
    };

    let start_tick = state.tick + 1;
    for tick in start_tick..=target_tick {
        let record = load_tick_commit_record(tick);
        let envelope = load_tick_input_envelope(tick);
        let transitions = load_replay_transitions(tick);

        apply_config_mod_deploy_admin_transitions(&mut state, transitions, &envelope);
        execute_deterministic(&mut state, record.commands, record.rejections, &envelope);
        assert_eq!(state.checksum(), record.state_checksum);
    }
...
If a verified keyframe or delta is missing, corrupted, or hash-mismatched, replay must discard that acceleration path and restart from genesis using the permanent redb replay log. `state.checksum() != record.state_checksum` → deterministic replay bug or corrupted log.
...
RichTraceBlob 缺失的 tick 标记 `terminal_state = audit_gap`——replay 可用 permanent redb genesis log 重建；keyframe/delta 仅作为 verified acceleration，仅 debug/rich trace 不可用。
...
给定 tick N-1 状态 + tick N RawCommand + world_seed + 激活模组列表 → `execute_deterministic` 产出的 checksum 必须等于 TickCommitRecord 的 `state_checksum`。

确定性依赖：
- PRNG：Blake3 XOF，确定性种子 + offset → 随机流，不依赖 OS 熵源
- Hash：Blake3 固定实现，不用 `std::hash`（跨版本可变）
- 排序：`(player_order, player_id, sequence, command_id)` — gameplay queue 仅包含玩家 WASM RawCommand；相同 seed + 相同玩家集 + 相同指令 → 相同顺序。详见 §9.1。
- ECS：`.chain()` 严格串行，`.before()/.after()` 部分并行
- 数值：整数 + 定点数，禁用 `f64`（跨平台/编译器非确定）。禁止 IEEE 754 浮点数——所有数值计算使用 `u64`/`i64` 定点整数（basis points, ×10000 精度），禁止任何浮点类型出现在游戏状态中。定点数在 Swarm codec 中使用整数表示（非小数），避免跨语言精度差异。
- **canonical Swarm codec**：`canonical_swarm_codec()` 使用 IDL-generated、versioned、little-endian、length-prefixed Swarm codec。对象/映射键按 IDL canonical order 排序；禁止多余 padding；canonical codec **不额外执行 Unicode NFC normalization**。`canonical_codec_version` 与 `serde_swarm`/`swarm-codec-go` 双实现 hash fixture 见 `specs/core/persistence-contract.md` §2.1。
- HashMap/Dictionary：`BTreeMap`，不用 `std::HashMap`（迭代顺序非确定）

### 7.2 回放验证

CI 对随机采样 tick 做 full replay 验证：`execute_deterministic(state, commands).checksum() != TickCommitRecord.state_checksum` → 确定性 BUG。
...
本节从 `design/engine.md` 下沉单 tick 各阶段资源预算。世界配置表示见 [`world-rules.md`](world-rules.md)，WASM 单次执行限制见 [`wasm-sandbox.md`](wasm-sandbox.md)，命令来源约束见 [`command-source.md`](../security/command-source.md)。
...
容量公式必须使用经 benchmark 校准的 `fuel_schedule_version → calibrated_fuel_per_core_ms`。默认 `worker_pool_max = 256`，可按负载调整，但每 shard active-player hard cap 固定为 500。cgroup `cpu.max` 与 sandbox deadline 的解耦保证单个坏玩家不能耗尽其他玩家的 COLLECT 时间窗。
...
本节从 `design/engine.md` 的确定性决策下沉可执行合同；engine、Gateway、SDK 与 replay verifier 必须一致实现。

### 9.1 命令全局排序键

所有 gameplay RawCommand 按**分层排序键**确定全局执行顺序：

```
sort_key = (player_order, player_id, sequence, command_id)
```

**第一层 `player_order`**：使用 Fisher-Yates 种子洗牌确定玩家顺序。种子 = `Blake3("shuffle" || world_seed || tick.to_le_bytes())`，TickCommitRecord 记录 `seed_epoch` 和活跃玩家集快照以支持回放。

**第二层 `player_id`**：稳定 tiebreaker。`seeded_shuffle` 内部先按 `PlayerId` 字节序排序，再执行 shuffle；排序键中保留 `player_id` 保证同一 `player_order` 下仍有唯一确定顺序。

**第三层 `sequence`**：同一玩家同一 tick 内按 `sequence` 升序排列（per-player 单调递增序号）。

**第四层 `command_id`**：`Blake3(canonical_swarm_codec(raw_command))` 作为稳定 tiebreaker，确保完全确定性排序（即使前三层相同）。`raw_command` 是服务端注入 player_id/tick/sequence/action 后的 canonical RawCommand。

Admin emergency/audit operations、`swarm_deploy` 与 MCP query 属于 control plane，不进入本排序键。

排序结果确定——相同 seed + 相同玩家集 + 相同命令集 → 相同执行顺序。此排序键同时用于 System Pass 2a inline apply 顺序和 TickCommitRecord 记录。
...
tick N+1:  若 redb deploy manifest 存在且 compiled artifact 匹配 → COLLECT 阶段加载新模块 → EXECUTE 阶段首次执行
           若 compiled artifact 缺失或 hash 不匹配 → drone 保持替换前模块，deploy 进入 FAILED
           object-store audit blob 状态不参与激活判定
```
...
| 消费端 | 读源 | 版本 | 滞后 |
|---|---|---|---|
| **WASM ABI v2 `tick(TickInput bytes)`** | COLLECT 开始时 Bevy snapshot | `TickInput.snapshot.tick == current_tick` | 0（本 tick 初始状态） |
...
**关键不变量**：WASM `tick()` 和 MCP `swarm_get_snapshot` 始终看到同一份 COLLECT-start 权威快照——不存在 COLLECT 内视图差异。WebSocket delta 不是该 snapshot：它在 tick N COMMIT 成功后由 committed state diff 计算，并在同一 tick N 的 BROADCAST 推送；这些变化首次进入玩家感知 snapshot N+1。
...
**body_cost refund 规则**：若 System Pass 2b spawn_system 创建失败（如 room cap 竞争），已扣除的 body_cost 全额退还到原扣费来源（`spawn.resources["Energy"]` 优先，capacity 不足部分回到全局存储）。refund 与 fuel refund 是独立资源池。

**事务大小约束**：小规模模式使用单个 <10MB redb transaction。Room-partition 模式先用不可见 staging transactions 写 content-addressed room rows，再用唯一的 <10MB `GlobalTickCommit` transaction 原子发布 head + manifest pointers；crash 前未发布 staging 由 GC 清理。两种模式的 authority point 都是一次 final atomic publish，COMMIT 总 p99 必须 ≤50ms。大型 RichTraceBlob/keyframe 写对象存储。
...
- **RichTraceBlob**：对象存储异步写入，写入失败 → `terminal_state = audit_gap`（不触发回滚）。缺失时 replay 可用 permanent redb genesis log 重建；keyframe/delta 仅作为 verified acceleration，仅 debug/rich trace 不可用。
- **ReplayArtifact**：从 permanent redb genesis log 生成：`TickCommitRecord`（critical subset）+ `TickInputEnvelope` + config/mod/deploy/admin transitions + optional verified keyframe/delta acceleration。`terminal_state = reconstructable` 时仍必须能回退到 genesis log。
...
TickCommitRecord (redb, same-tx)  ──→  不可降级，失败 = abandon
        ...
        └──→ 与 TickInputEnvelope + transitions → ReplayArtifact
             （keyframe/delta 仅作 verified acceleration）
...
系统执行顺序由 `design/engine.md` 决定，并在 [Complete Tick Execution Manifest](phase2b-system-manifest.md) §1 下沉。以下是 25 个 registered Pass 2b systems 的 serial spine + 2 parallel sets（加上 6 个 Pass 2a inline handlers 为 31 combined）：

```
death_marker → spawn → spawning_grace → regeneration →
combat (parallel set A: attack/ranged_attack/heal) →
special_attack_reducer → damage_application (including Leech actual-damage self-heal) →
status/resolution buffer production (parallel set B: hack_buf/drain_buf/overload_buf/debuff_buf/disrupt_buf/fortify_buf/leech_trace_buf/fabricate_channel_buf) →
...
Persistent special 状态由 `status_advance_system`（S22）**唯一**串行处理——S16-S22b 只写 typed buffer。A01 action dispatch 写入 special intent；S14 归并裁决后把 Leech 路由到 S15，把 Fabricate/persistent status intents 路由到 S22。Leech 不存在 `LeechState`；Fabricate 是 5-tick interruptible `FabricateState` channel，完成时通过 `PendingEntityCreation` 替换目标。

**关键变更**：
- `regeneration`（S10）移至 `damage_application`（S15）之前，防止 heal+regen 双倍回复。
- `spawning_grace`（S09）移至 combat（S11-S13）之前，确保首次可交互 tick 免疫保护。
...
| **Truncated** | output > 256KB | 整批丢弃，产出非 wire 状态 `output_truncated` |
...
WASM 模块通过 `wasm_output_status: "output_truncated"` 收到状态通知，下一 tick 可重新输出缩减后的指令列表。该值不是 canonical `RejectionReason`。引擎不执行前缀解析——杜绝部分执行导致的游戏状态不一致。
