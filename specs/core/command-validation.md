...
Action handler 在 Phase 2a 中只写 typed intent buffer 或非 HP 资源/event；combat HP 变化统一由 S15 `damage_application` 写入，status 变化统一由 S22 `status_advance_system` 写入。handler 禁止直接修改 `HitPoints` 或 `StatusState`。

### 3.8 特殊攻击状态机矩阵

#### 同 tick 多命中优先级

> **R24 B3-GAP 修复**：特殊攻击优先级以 `phase2b-system-manifest.md` §S14 为唯一权威。此处不再重列可冲突的优先级顺序。
...
**调度位置**: `status_advance_system` 的权威位置在 `specs/core/phase2b-system-manifest.md` §S22。该 manifest 是 Phase 2b 系统调度的**唯一权威**——此文档不重复列完整调度链，以免与 manifest 产生冲突。任何调度相关问题以 manifest 为准。
