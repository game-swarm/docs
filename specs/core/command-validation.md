...
**单一 gameplay 管线**：默认 gameplay command 入口是 WASM `tick()` 输出的 `CommandIntent[]`；经授权的 Admin emergency/audit command 也走同一 `校验 → 应用` 路径。MCP deploy/query 属于控制面，不进入 gameplay RawCommand queue。
 ...
 **`sequence` 排序范围**：`sequence` 是 **per-(player, source)** 级别——同一玩家在同一 tick 内通过同一 gameplay source 提交的指令按 `(source, sequence)` 排序。全局排序键：`(priority_class, shuffle_index, source_rank, sequence, command_hash)`；`priority_class` 仅包含 Admin 与 WASM，`shuffle_index` 来自 PRNG 的确定性 shuffle（种子 = `Blake3("shuffle" || world_seed || tick.to_le_bytes())`），`command_hash` = `Blake3(canonical_serialize(raw_command))` 作为稳定 tiebreaker。MCP deploy/query 属于 control plane，不进入该排序键。`canonical_serialize()` 遵循 RFC 8785/JCS；不额外要求 Unicode NFC normalization。使用 RawCommand（非 CommandIntent 原始输出）——确保 player_id/source/tick 等注入字段参与 hash。详见 `tick-protocol.md` §9.1。
...
| `source` | Source | **服务端注入** | gameplay queue 仅接受 WASM 与经授权的 Admin source |
...
> **注意**：以下校验表中的错误码为说明性名称。权威 `RejectionReason` enum 由 IDL YAML 与 [API Registry §2](../reference/api-registry.md#2-rejectionreason) 同步维护。`Fatigued`、`NotMovable`、`SourceEmpty`、`TargetFull`、`TargetEmpty` 等上下文信息放入 `debug_detail` 字段。下文表格中标注 `(debug_detail)` 的表示该条件不映射到独立 canonical code。
...
```json
{"type": "Move", "object_id": 1001, "direction": "Top"}
```
...
| Direction 是合法六方向 hex 邻居 (`Top` / `TopRight` / `BottomRight` / `Bottom` / `BottomLeft` / `TopLeft`) | `InvalidDirection` |
...
Combat/effect action 通过 ActionRegistry dispatch。以下各 action 的详细参数以 [Vanilla Action Canonical Table](../reference/special-attack-table.md) 和 [API Registry §1.4 ActionRegistry](../reference/api-registry.md#14-actionregistry--11-vanilla--mod-extensible-combat-actions) 为权威源。Leech/Fabricate 的注册方式已从 `[[custom_actions]]` 改为 ActionRegistry vanilla action——非 mod extension。

#### Disrupt

```json
{ "type": "Disrupt", "object_id": "d1", "target_id": "e5" }
```
...
```json
{ "type": "Fortify", "object_id": "d1", "target_id": "f2", "sequence": N }
```
...
```json
{ "type": "Hack", "object_id": "d1", "target_id": "e5", "sequence": N }
```
...
```json
{ "type": "Drain", "object_id": "d1", "target_id": "s1", "sequence": N }
```
...
```json
{ "type": "Overload", "object_id": "d1", "target_id": 42, "sequence": N }
```
...
```json
{ "type": "Debilitate", "object_id": "d1", "target_id": "e5", "damage_type": "Kinetic", "sequence": N }
```
...
```json
{ "type": "Leech", "object_id": "d1", "target_id": "e5", "sequence": N }
```
...
```json
{ "type": "Fabricate", "object_id": "d1", "target_id": "e5", "sequence": N }
```
