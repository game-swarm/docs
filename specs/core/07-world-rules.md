...
Rhai 模组通过声明式钩子接入引擎生命周期。每个钩子在确定的 tick 阶段触发——模组在 `mod.toml` 中声明 `hooks = [...]`，引擎按顺序调用。

**可用钩子**：

| 钩子 | 触发点 | 频率 | 函数签名 | 说明 |
|------|--------|:----:|---------|------|
| `on_world_init` | 世界启动（tick 0 前） | 1次 | `fn(world_cfg, mod_cfg)` | 初始化模组状态、设置初始参数 |
| `on_tick_start` | COLLECT 阶段前 | 每 tick | `fn(world_cfg, tick, mod_state)` | 预处理：调整税率、设置临时规则 |
| `on_tick_end` | BROADCAST 阶段后 | 每 tick | `fn(world_cfg, tick, events, mod_state)` | 后处理：计算统计、触发延迟事件 |
| `on_command_validated` | Phase 2a 校验通过后 | 每命令 | `fn(cmd, world_cfg, mod_state) → bool` | 返回 `false` 阻止应用（RejectionReason → ModBlocked） |
| `on_command_applied` | Phase 2a 命令应用成功后 | 每命令 | `fn(cmd, result, world_cfg)` | 审计、计数、触发连锁反应 |
| `on_spawn` | Phase 2b spawn_system | 每 drone | `fn(drone_id, body, room, mod_state)` | 修改新生 drone 属性、附加 flag |
| `on_death` | Phase 2b death_mark_system | 每 drone | `fn(drone_id, cause, killer_id?, mod_state)` | 死亡掉落、击杀统计、复仇标记 |
| `on_room_claim` | Controller owner 变更 | 每事件 | `fn(room_id, old_owner, new_owner, mod_state)` | 领土变更通知、边境事件 |
| `on_deploy` | WASM 模块部署成功 | 每部署 | `fn(player_id, module_hash, slot, mod_state)` | 部署审计、部署限制 |
...
引擎按上述列表顺序调用——`mod.toml` 中声明顺序无关（引擎固定调度）。未声明的 hook 不调用（零开销）。
...
Rhai 脚本通过 `query.*` 命名空间读取世界状态（只读），通过 `actions.*` 写入（见 §5.1 能力命名空间）。
...
### 5.1d Capability 白名单（R27 CX3）

扩展 `actions.*` 能力需在 `mod.toml` 中声明 `required_capabilities`，服主在 `world.toml` 中按模组授权。
...
不写 `capabilities` = 全部已声明 capability 均授权。未声明 capability 即使授权也不可调用（安全优先）。

### 5.1e 错误传播与降级（R27 CX3）
