 ...
 **`sequence` 排序范围**：`sequence` 是 **per-(player, source)** 级别——同一玩家在同一 tick 内通过同一 source 提交的指令，按 `(source, sequence)` 排序。不同 source 的 `sequence` 空间独立（WASM 的 seq=1 不与 MCP_Deploy 的 seq=1 冲突）。全局排序键：`(priority_class, shuffle_order, source, sequence)`，其中 `shuffle_order` 来自 PRNG 的确定性 shuffle（种子 = `Blake3("shuffle" || world_seed || tick.to_le_bytes())`），`priority_class` 为 Admin > NPC > Player。详见 `01-tick-protocol.md` §9.1。
