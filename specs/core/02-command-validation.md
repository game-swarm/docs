...
**Spawn body_cost 扣除时点**：`body_cost` 在 Phase 2a spawn 命令校验时**立即扣除**（inline apply）——此时 spawn 尚未创建，body_cost 已从 spawn.energy 和/或全局存储中扣除。若 Phase 2b spawn_system 创建失败（如 room cap 竞争），已扣除的 body_cost **全额退还**到原扣费来源（spawn.energy 优先，其次全局存储）。此 refund 记入 `next_tick_fuel_credit`（见 §7.2），不计入当前 tick 的 fuel budget。
