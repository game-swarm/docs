...
> **注意**：以下校验表中的错误码为说明性名称。权威 `RejectionReason` enum 共 47 个 canonical code（35 game + 12 auth，见 [API Registry §2](../reference/api-registry.md#2-rejectionreason)）。D2/B 设计决策：`Fatigued`、`NotMovable`、`SourceEmpty`、`TargetFull`、`TargetEmpty` 等旧码已从 wire enum 移除，其上下文信息放入 `debug_detail` 字段。下文表格中标注 `(debug_detail)` 的表示该条件不映射到独立 canonical code。
...
--------|--------|
 ...
 `body.len() ≤ MAX_BODY_PARTS (50)` | `NotEnoughBodyParts` |
 `body_cost(body) ≤ spawn.energy` | `InsufficientResource` |
 | `body.loc() ≤ 玩家房间能量上限` | `InsufficientResource`（见 [API Registry §2.6](../reference/api-registry.md#26-validation-condition--rejectionreason--debug_detail-映射)） |
...
------|------|------|
 ...
 | MAX_DRONES_PER_PLAYER | 50 | 可配置（world.toml 中 `drone.max_drones_per_player`），默认 50。基准容量目标: 50 players × 10 drones = 500 total |
