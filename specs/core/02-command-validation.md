...
> **注意**：以下校验表中的错误码为说明性名称。权威 canonical `RejectionReason` enum 共 35 个 code（见 [API Registry §2](../reference/api-registry.md#2-rejectionreason)）。D2/B 设计决策：`Fatigued`、`NotMovable`、`SourceEmpty`、`TargetFull`、`TargetEmpty` 等旧码已从 wire enum 移除，其上下文信息放入 `debug_detail` 字段。下文表格中标注 `(debug_detail)` 的表示该条件不映射到独立 canonical code。
