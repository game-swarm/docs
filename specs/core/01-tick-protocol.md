...
### 3.5 Tick 原子性 — Room-Partition 提交
...
struct GlobalTickCommit {
    tick: u64,
    per_room_commit_hashes: Vec<(RoomId, Blake3)>,  // 每房间 commit hash
...
```
处理流程:
  1. 收集 cross_room_intent_set（按确定性顺序：room_id source 升序）
  2. 对每个 intent:
     ├─ source room commit 成功 + target room commit 成功 → intent 执行
...
**回滚规则**：
- 跨房间 intent 失败 → 相关房间的 per-room 状态回滚至 tick 前快照
- Snapshots（每房间）在 Phase 2a 开始前捕获
