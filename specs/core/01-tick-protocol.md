...
生产环境使用 **shadow write + atomic publish** 模型保证 tick 原子性——将世界按房间分片写入 FDB staging 区，仅通过 GlobalTickCommit 一次性发布。整个 tick 是**全局原子**的：staging 写入失败不产生已持久化的中间状态，GlobalTickCommit 是唯一的提交点。
...
**关键区别 vs 旧模型**：
- **旧（已删除）**：每房间独立 FDB 事务提交 + 全局回滚。声称「room-partition 仅是写入分片策略，不产生 per-room 独立 commit 语义」，但实现上仍依赖 per-room commit hash → 全局 manifest——存在「per-room 写入已持久化、全局 abort」的时序窗口。
- **新（Shadow Write）**：Per-room 写入只到 `/staging/{tick}/{room}`——**staging 行不是已提交状态**。GlobalTickCommit 是唯一的 publish 点——它将 staging 行原子提升为 `/committed/{tick}/{room}`（或等价的一次性 pointer swap），写入全局 head + room hashes + manifest hash。所有下游读取（replay、Dragonfly、MCP query）仅走 `/committed/` 路径——staging 中的数据对外不可见。
...
Phase 2b 完成后:
  ...
  1. 对每个活跃房间：独立 FDB 事务写入 staging 键空间
     ├─ Key: /staging/{tick}/{room_id}/state   → 房间状态 delta
     ├─ Key: /staging/{tick}/{room_id}/events  → 房间内事件（commands/rejections）
     └─ room_txn.commit() → per_room_staging_hash = Blake3(staging_content)
  ...
  4. GlobalTickCommit（唯一 publish 点 — FDB 原子事务）:
     ├─ 写入 /committed/head/{tick}            → global tick head
     ├─ 写入 /committed/manifest/{tick}         → room hashes + cross-room intent log
     ├─ 写入 /committed/hash_chain/{tick}       → hash chain 续接
     ├─ staging → committed promotion           → staging 行逻辑提升（或 pointer swap）
  ...
  **关键不变量**：
  ...
  - 不存在「已持久化但未发布」的游戏状态——所有 `/committed/` 路径下的数据保证是完整 tick。
  - Staging 行仅 GlobalTickCommit 在同一事务中读取——无 TOCTOU 窗口。
```
...
Staging GC 策略:
  ...
  - GlobalTickCommit 成功 → staging 行在 promotion 时逻辑删除（复用 or 清空）。
  - GlobalTickCommit 失败 → staging 行成为孤立数据。
    GC worker（每 10s 扫描）:
      └─ 读取 /committed/head/{tick}：若不存在（tick 未发布），清理 /staging/{tick}/**。
...
struct GlobalTickCommit {
    tick: u64,
    per_room_staging_hashes: Vec<(RoomId, Blake3)>,  // 每房间 staging 内容 hash
    ...
    staging_gc_list: Vec<StagingKey>,                   // 需 GC 的 staging 行（本 tick isolated）
}
```
...
| **TickCommitRecord write fail** | FDB 事务内 TickCommitRecord 写入失败 | tick 放弃（同事务，state 回滚） | fuel 退还 | tick abandon，Bevy snapshot 恢复。不存在"状态已提交但审计记录缺失"的缺口 |
...
**TickCommitRecord**（FDB replay-critical，同事务）：
TickCommitRecord 写入 FDB 与 tick 执行在**同一事务**中——写入失败 = 事务回滚 → tick 放弃。
...
**WAL 恢复**：WAL 中的 TickCommitRecord 在 FDB 恢复后异步回放写入——不阻塞 tick 执行。WAL 保留策略：最多 10,000 tick 或 24h（先到者清理）。

**审计完整性保证**：
- tick 执行与 TickCommitRecord 写入是**同一 FDB 事务**——要么都成功，要么都失败
- FDB 事务冲突回滚时，TickCommitRecord **不**写入——避免"有审计记录但世界状态未变"的不一致
- 连续 3 次 FDB 写入失败 → tick 放弃（见 §6.1），TickCommitRecord 同样放弃——不存在"世界状态已变但无审计记录"的缺口
...
RichTraceBlob 在 FDB commit **成功**后异步写入对象存储——写入失败**不回滚**已提交的 tick。

| 失败场景 | 行为 | 语义 |
|---------|------|------|
| 初次写入失败 | 重试最多 3 次（指数退避 1s/2s/4s） | 不阻塞 tick 循环 |
| 3 次重试后仍失败 | `tick_manifest.upload_status = "failed"` | `terminal_state = audit_gap`；FDB 状态完整 |
...
- **canonical JSON**：`canonical_serialize()` 遵循 **[RFC 8785 JSON Canonicalization Scheme (JCS)](https://www.rfc-editor.org/rfc/rfc8785)**。禁止 IEEE 754 浮点数编码（JCS §3.2.2 定义的数字格式仅限整数，引擎侧不使用 `f64`/`f32`，输出端无浮点 JSON 数字）。对象键按 UTF-8 字节序字典序排列；字符串中 Unicode 转义规范化（JCS §3.2.3）；禁止多余空白字符。`canonical_codec_version` 与 `serde_swarm`/`swarm-codec-go` 双实现 hash fixture 见 `specs/core/05-persistence-contract.md` §2.1。
...
| **Replay / TickCommitRecord** | FDB 持久化记录 | `tick_commit_record.tick == executed_tick` | 0（权威审计记录） |
...
TickCommitRecord 必须与 FDB 状态写入在同一事务中——禁止"状态成功但审计不完整"与"同事务无缺口"并存：
...
| EXECUTE 完成，FDB commit 前 | 是 | 是 | FDB 事务未提交 → state + TickCommitRecord + fuel 三者均未持久化 → 自动回滚，下一 tick 重试 |
...
**事务大小约束**：单 tick 的 FDB 事务仅写入状态 delta + TickCommitRecord manifest + fuel 记录，不写入全量世界状态。大型 binary payload（RichTraceBlob、keyframe snapshot）写入对象存储，FDB 仅存指针 + hash + 大小。确保事务大小 < 10MB（FDB 推荐上限）。跨 tick 状态通过 prior_tick manifest hash 形成可验证链。

**完整性保护（分层语义）**：
- **TickCommitRecord**：FDB 同事务原子写入，缺失 ≥1 字段则 tick 不可 replay（`terminal_state = unreplayable`）。tick 放弃时 TickCommitRecord 不产生——不存在"状态已变更但无审计记录"。
...
三者关系：
```
TickCommitRecord (FDB, same-tx)  ──→  不可降级，失败 = abandon
        │
