...
- 排序：`(priority_class, shuffle_index, sequence, source)` — 相同 seed + 相同玩家集 + 相同指令 → 相同顺序。详见 §9.1。
...
```
sort_key = (priority_class, shuffle_index, sequence, source)
```

**第一层 `priority_class`**：命令优先级类别（Admin > NPC > Player）。Admin 命令始终最先（紧急冻结/审计操作），NPC 在玩家之前（世界模拟），玩家间进入下一层。
...
**第三层 `sequence`**：同一玩家内按 `sequence` 升序排列（per-player 单调递增序号）。

**第四层 `source`**：同一 sequence 内按 `source` 排序（`WASM` > `MCP_Deploy`），作为最终 tiebreaker。
