...
> 详见 design/engine.md
>
> **修复**。本文档定义 Swarm 引擎的持久化分层架构，消除 "redb WriteTransaction 内写一切" 与 "跨存储双写会炸" 之间的合同空白。
>
...
| **Blob Store** | RichTraceBlob、snapshot delta (full/diff)、replay artifacts、WASM binaries | < 10MB/object | `world.toml` retention policy |
| **WAL (Write-Ahead Log)** | 未提交的 apply 操作日志 | 滚动 | 提交后截断 |
| **Keyframe Store** | 每 K tick 的完整世界状态快照 | < 100MB | `world.toml` retention policy，primary + backup |

---

## 2. Replay-Critical Subset（权威声明）

replay-critical 字段在 redb WriteTransaction 中原子提交（replay-critical），哪些可以异步写入对象存储（debug/rich）。此声明是 `persistence-contract.md` 的最权威条款——所有其他文档引用 persistence 合同时以此为准。
...
| 9 | `manifest_hash` | redb tick_manifest | core systems manifest、plugin schedule graph、ActionRegistry 与 host fuel schedule version hash |
...
COMPILE_PREPARE:
  ├─ 验证 deploy body 中的 wasm_module_hash
...
Replay 与 rich artifact 保留期由 `world.toml` 配置，使用 tick 数，不使用天数作为存储层权威值：

```toml
[retention]
deterministic_replay_retention_ticks = 5_184_000   # Arena 默认约 180d @ 3s/tick
rich_artifact_retention_ticks = 864_000            # 可独立设置
keyframe_backup_copies = 2                         # primary + backup
```

`deterministic_replay_retention_ticks` 控制 redb replay-critical core、keyframe 与 delta chain 的可回放窗口。`rich_artifact_retention_ticks` 控制 RichTraceBlob、可视化 annotation 与调试 artifact；它可以短于 deterministic replay，不影响确定性回放。
...
**孤儿清理**: 由于 redb commit 先于 blob 写入，正常流程不产生孤儿 blob。唯一孤儿场景：blob 写入成功但 etag 回填失败 → GC 扫描 `archive_status != 'complete'` 但对象存储中存在对应 blob → 保留 1h 后删除。**正常 `archive_status = 'complete'` 的 blob 按 TTL 分层清理**（见上表）。

### 6.3 Keyframe GC

- keyframe 与 deterministic delta chain 至少保留 `deterministic_replay_retention_ticks`
- 每个 keyframe 至少写入 2 份冗余：primary + backup，位于不同故障域
- 删除 keyframe 时同步删除对应的 snapshot delta chain；删除不得破坏配置窗口内任一 tick 的 replay
...
> **裁决**：redb room-partition transaction 纳入核心合同。单事务模式仅支持小规模验证（≤ 50 active players, ≤ 100 rooms）；500/1000-player 场景必须使用 room-level partition（Shadow Write）。
...
保留: 至少 `deterministic_replay_retention_ticks`
```
...
设计恢复目标：RPO ≤ 100 ticks（5 min @ 3s/tick），RTO ≤ 300s。redb 保存当前权威状态与配置 retention 窗口内 replay-critical core；历史全量世界状态不保存在 redb，完整恢复锚点由独立 Keyframe Store 提供。
