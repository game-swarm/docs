...
> **本文档是临时性进度追踪文件——不作为设计评审目标。** 设计合同见 design/README.md + specs/。
>
...
| Blocker | 内容 | 涉及文件 |
|---------|------|---------|
| B1 | Overload: 可见性约束、全局冷却、静默结果、短期压制恢复 | specs/core/02, specs/gameplay/08 |
| B2 | Spec Convergence: Direction 正方形、命令上限 500、spectate_delay≥50、IDL/Manifest 拆层 | specs/core/02, specs/gameplay/08, specs/security/05 |
| B3 | Tick semantics: Phase 2a TOCTOU 合同、并行 RW 矩阵 | specs/core/01 |
| B4 | 部署签名: 客户端 Ed25519 + nonce/CRL/epoch | specs/security/09 |
| B5 | 输出面: Browser/Agent transport 拆分、DNS rebinding | specs/security/03 |
| B6 | 资源边界: snapshot 256KB、simulate caps、audit 截断 | specs/core/01, specs/core/04 |
| B7 | 用户裁决: Vanilla 分层(特殊攻击默认禁用)、Arena 房间制 | design/README.md |
...
| ID | 裁决 | 影响范围 |
|----|------|---------|
| D-1 | Rhai inprocess + 强制 Ed25519 数字签名（删除 out-of-process 模式） | specs/core/07 |
| D-2 | World 模式无胜利条件（MMO 沙盒） | design/README.md §9 |
| D-3 | 新生 drone SpawningGrace 1 tick 无敌帧 | design/README.md, specs/core/01, specs/core/02 |
| D-4 | Tier 2/3 快照扩展必须 spec-ready（不延后） | design/README.md, tech-choices, 本 ROADMAP |
| D-H1 | First-hour onboarding: 渐进威胁曲线 + soft_launch + AI 首次部署引导 + 低风险社交冲突 | specs/gameplay/06 |
| PvE | World PvE 生态层 (NPC/据点/事件/掉落/难度梯度) + Arena PvE Challenge 模式 (4 场景 + 评分) | design/README.md §9.0, §9.1.5 |
| A-H3 | Engine 启动序列 + RUNBOOK 补全 (启动顺序约束/降级启动/启动后验证) | RUNBOOK.md |
| S-H2 | TickTrace 审计完整性 — FDB 同事务写入 + WAL 降级链 | specs/core/01 §6.3.4 |
| S-H3 | WASM sandbox OS 边界加固 checklist (seccomp/cgroup/namespace/CI) | specs/core/04 §9 |
| D-H3 | Move-as-action 设计理由 (确定性/编程游戏本质/战术深度) | design/README.md §3.2 |

### Tier 2/3 快照扩展 — 待 spec

| 项目 | 内容 | 状态 |
|------|------|:--:|
| Tier 2 spec | 增量快照协议、CoW 实体分页、modification-set 合并、truncation 增量语义 | 🟡 stub — `specs/future/T2-incremental-snapshot.md`（Phase 1+ 冻结） |
| Tier 3 spec | 分片键设计、跨分片实体引用、分布式 combat 结算、FDB 多区域部署 | 🟡 stub — `specs/future/T3-shard-protocol.md`（Phase 1+ 冻结） |
...
### S2: 缺失 RejectionReason 变体 — specs/core/02 §3.10-3.13 ✅
...
### S1: COLLECT 结果跨 FDB 重试缓存 — specs/core/01 §3.5 ✅
...
### S7a: code_propagation_system 空实现 — specs/core/07 §3 ✅
...
### S7b: memory_upkeep_system 未注册 — specs/core/07 §3 ✅
...
### S7c: drone_env_var_system 未实现 — specs/core/07 §3 ✅
...
### S6b: Tutorial 引导系统 — specs/gameplay/06 §2.1 ✅
...
| Spec | 状态 |
|------|------|
| specs/core/01 | ✅ 核心 tick 协议完整（除 COLLECT 缓存） |
| specs/core/02 | ✅ 指令管线完整（除 5 个 RejectionReason） |
| specs/security/03 | ✅ MCP 工具 + OAuth2 + 限流全部对齐 |
| specs/core/04 | ✅ WASM 沙箱（Wasmtime/seccomp/cgroup/WASI）完整对齐 |
| specs/security/05 | ✅ 可见性（is_visible_to/fog/player_view/replay_privacy）完整对齐 |
| specs/gameplay/06 | ✅ 回放查看器 + 策略仪表盘已实现（除 Tutorial + sim CLI） |
| specs/core/07 | ✅ world.toml 解析 + 资源系统 + 模组加载 + 4 个 system 全部实现 |
| specs/gameplay/08 | ✅ IDL CommandAction/BodyPart/DamageType/Direction 枚举完整对齐 |
| specs/security/09 | ✅ CommandSource + SourceGate + Auth Context 完整对齐 |
