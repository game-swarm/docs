# R13 Game Designer Review — Claude Opus

> 零历史上下文评审。审查范围：DESIGN.md + specs/p0/。

## VERDICT: REQUEST CHANGES

存在 1 项 Critical（Hack 实际无法触发的死锁）与 3 项 High（RangedAttack 死参数、负面状态无净化反制、lifespan 续期 farming），这些直接影响 PvP 平衡与核心玩法成立性，需在进入下一阶段前修订。Medium/Low 多为数值与语义澄清，可随修订一并处理。

## Strengths

- **身体部件 + 指令系统的组合深度**：MOVE/ATTACK/HEAL/CLAIM 等部件与多 tick 指令（MoveTo/Attack/Heal）的组合，提供了 Screeps 系经典的高自由度策略空间，自定义 drone 配置是扎实的玩法地基。
- **特殊攻击的资源-冷却约束设计方向正确**：为 Hack/Debilitate/Overload/Drain 设置 Energy 成本 + 冷却 + 专用部件列，遏制了特殊攻击的无脑刷屏，框架是对的（只是 Hack 触发条件需要重调）。
- **伤害类型 + 抗性矩阵的可配置性**：6 种伤害类型经 world.toml 配置，配合 Tough/Fortify 的减伤层，为构筑克制关系预留了丰富设计维度。
- **可见性分层清晰**：drone 感知（fog_of_war）与玩家视野（player_view）分离，回放隐私分级（private/allies/world/public）独立，可见性模型在设计层面思考完整。
- **Arena 对称初始条件的公平承诺**：将竞技模式与持久世界分离、强调对称起手，是正确的竞技化方向（仅需补 world.toml 规则白名单约束）。
- **教程的引导式代码修改设计**：用 1s tick + 引导式编辑降低新手门槛的意图清晰（仅需补物流/维护费的渐进过渡）。
